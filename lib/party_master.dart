...
class _PartyMasterViewState extends State<PartyMasterView> {
  ...
  void _confirmDelete(PharoahManager ph, Party p) { if (p.name == "CASH") return; showDialog(context: context, builder: (c) => AlertDialog(title: const Text("Delete Party?"), content: Text("Are you sure?"), actions: [TextButton(onPressed: () => Navigator.pop(c), child: const Text("NO")), ElevatedButton(onPressed: () { ph.deleteParty(p.id); Navigator.pop(c); }, child: const Text("YES, DELETE"))])); }
}
