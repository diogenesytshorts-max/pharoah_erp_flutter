...
class ProductMasterView extends StatefulWidget {
  ...
  final Map<String, dynamic>? preFillData; // NAYA: Data carrier for Import
...
class _ProductMasterViewState extends State<ProductMasterView> {
  ...
  void initState() {
    super.initState();
    // Agar import screen se data aaya hai toh seedha form kholo
  ...
  void _showProductForm({Medicine? med}) async {
    ...
    // Data Source: 1. Existing Med (Edit), 2. PreFill (Import), 3. Empty (New)
    ...
    final purRateC = TextEditingController(text: med?.purRate.toString() ?? pf?['purRate']?.toString() ?? "0");
    final rateAC = TextEditingController(text: med?.rateA.toString() ?? pf?['saleRate']?.toString() ?? "0");
    ...
    // Agar import se Company Name aaya hai, toh usey apne master mein dhoondho
    if (pf != null && med == null) {
       if (pf['mfg'] != null) {
         try {
         ...
         } catch(e) { companyName = pf['mfg'].toString().toUpperCase(); } // Naya Name set kar diya
       }
       if (pf['salt'] != null) {
         try {
    ...
    showDialog(
      ...
      builder: (c) => StatefulBuilder(builder: (context, setDialogState) {
        return AlertDialog(
          ...
          title: Text(med == null ? "Register Imported Product" : "Edit Product Details"),
          content: SizedBox(
            width: 500,
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () {
                ...
                // Final Link Check: Agar Name se company create karni pade
                ...
                final newItem = Medicine(
                  ...
                  companyId: companyId ?? "",
                  saltId: saltId ?? "",
                  hsnCode: hsnC.text.isEmpty ? "3004" : hsnC.text,
