...
// 1. Batch Info Model (Batch history ke liye)
class BatchInfo {
...
class Medicine {
  ...
  factory Medicine.fromMap(Map<String, dynamic> map) => Medicine(
    ...
    manufacturer: map['manufacturer']??"N/A", hsnCode: map['hsnCode']??"N/A", 
    gst: (map['gst']??12).toDouble(), mrp: (map['mrp']??0).toDouble(), 
    purRate: (map['purRate']??0).toDouble(), rateA: (map['rateA']??0).toDouble(), 
    rateB: (map['rateB']??0).toDouble(), rateC: (map['rateC']??0).toDouble(), 
    stock: map['stock']??0
  );
}

// 4. Party / Distributor Master Model
class Party {
  ...
  Party({
    ...
    this.route = "", this.phone = "", this.email = "", this.dl = "N/A", 
    this.gst = "N/A", this.rateType = "A", this.openingBalance = 0.0, 
    this.specialDiscount = 0.0
  });

  Map<String, dynamic> toMap() => {
    'id': id, 'name': name, 'address': address, 'city': city, 'route': route, 
    'phone': phone, 'email': email, 'dl': dl, 'gst': gst, 'rateType': rateType, 
    'openingBalance': openingBalance, 'specialDiscount': specialDiscount
  };

  factory Party.fromMap(Map<String, dynamic> map) => Party(
    id: map['id']??"", name: map['name']??"", address: map['address']??"", 
    city: map['city']??"", route: map['route']??"", phone: map['phone']??"", 
    email: map['email']??"", dl: map['dl']??"N/A", gst: map['gst']??"N/A", 
    rateType: map['rateType']??"A", openingBalance: (map['openingBalance']??0.0).toDouble(), 
    specialDiscount: (map['specialDiscount']??0.0).toDouble()
  );
}

// 5. Bill Item Model (For Sales)
class BillItem {
  ...
  BillItem({
    ...
    required this.mrp, required this.qty, required this.rate, 
    this.discountPercent = 0.0, this.discountRupees = 0.0, required this.gstRate, 
  ...
  Map<String, dynamic> toMap() => {
    ...
    'discountPercent': discountPercent, 'discountRupees': discountRupees, 
    'gstRate': gstRate, 'cgst': cgst, 'sgst': sgst, 'total': total
  };

  factory BillItem.fromMap(Map<String, dynamic> map) => BillItem(
    id: map['id']??"", srNo: map['srNo']??0, medicineID: map['medicineID']??"", 
    name: map['name']??"", packing: map['packing']??"", batch: map['batch']??"", 
    exp: map['exp']??"", hsn: map['hsn']??"", mrp: (map['mrp']??0).toDouble(), 
    qty: (map['qty']??0).toDouble(), rate: (map['rate']??0).toDouble(), 
    discountPercent: (map['discountPercent']??0).toDouble(), 
    discountRupees: (map['discountRupees']??0).toDouble(), 
    gstRate: (map['gstRate']??0).toDouble(), cgst: (map['cgst']??0).toDouble(), 
    sgst: (map['sgst']??0).toDouble(), total: (map['total']??0).toDouble()
  );
}
...
class Sale {
  ...
  Map<String, dynamic> toMap() => {
    ...
    'paymentMode': paymentMode, 'totalAmount': totalAmount, 'status': status, 
    'items': items.map((i) => i.toMap()).toList()
  };
}

// 7. Purchase Item Model (For Stock Entry)
class PurchaseItem {
  String id, medicineID, name, packing, batch, exp, hsn;
  int srNo;
...
class Purchase {
  ...
  Map<String, dynamic> toMap() => {
    ...
    'totalAmount': totalAmount, 'items': items.map((i) => i.toMap()).toList()
  };
  ...
  factory Purchase.fromMap(Map<String, dynamic> map) => Purchase(
    ...
    date: DateTime.parse(map['date']), totalAmount: (map['totalAmount']??0).toDouble(), 
