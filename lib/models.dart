...
// ===========================================================================
// 1. SYSTEM SETTINGS & CONFIG MODELS
// ===========================================================================

class NumberingSeries {
  String id;
  String name;        // e.g., "Cash Sale Series"
  String type;        // SALE, PURCHASE, CHALLAN, RETURN, VOUCHER
  String prefix;      // e.g., "CUSA-", "INV/"
  int startNumber;    // e.g., 1 or 1001
  bool isDefault;     // Is it the primary series for this type?
  bool isActive;      // To STOP a series without deleting history
  ...
  factory NumberingSeries.fromMap(Map<String, dynamic> map) {
...
// ===========================================================================
// 2. MASTER MODELS (Route, Company, Salt, DrugType, Bank, Salesman)
// ===========================================================================
...
class Bank { String id, name, branch, accountNo; double openingBalance; Bank({required this.id, required this.name, this.branch = "", this.accountNo = "", this.openingBalance = 0.0}); Map<String, dynamic> toMap() => {'id': id, 'name': name, 'branch': branch, 'accountNo': accountNo, 'openingBalance': openingBalance}; factory Bank.fromMap(Map<String, dynamic> map) => Bank(id: map['id'] ?? "", name: map['name'] ?? "", branch: map['branch'] ?? "", accountNo: map['accountNo'] ?? "", openingBalance: (map['openingBalance'] ?? 0.0).toDouble()); }
class Salesman { String id, name, phone, route; Salesman({required this.id, required this.name, this.phone = "", this.route = ""}); Map<String, dynamic> toMap() => {'id': id, 'name': name, 'phone': phone, 'route': route}; factory Salesman.fromMap(Map<String, dynamic> map) => Salesman(id: map['id'] ?? "", name: map['name'] ?? "", phone: map['phone'] ?? "", route: map['route'] ?? ""); }
...
class Party {
  ...
  Party({
    required this.id, required this.name, this.group = "Sundry Debtors", this.phone = "", this.email = "", 
    this.address = "", this.city = "", this.state = "Rajasthan", this.route = "", this.gst = "", 
    this.dl = "", this.dlExp = "", this.pan = "", this.transport = "", this.priceLevel = "A", 
    this.defaultSeriesId = "", this.opBal = 0.0, this.creditLimit = 0.0, this.creditDays = 0
  });
...
class PurchaseReturn { String id, billNo, distributorName, status, returnType; DateTime date; List<PurchaseItem> items; double totalAmount; PurchaseReturn({required this.id, required this.billNo, required this.distributorName, required this.items, required this.totalAmount, required this.date, this.status = "Active", this.returnType = "Sellable"}); Map<String, dynamic> toMap() => {'id': id, 'billNo': billNo, 'date': date.toIso8601String(), 'distributorName': distributorName, 'totalAmount': totalAmount, 'status': status, 'returnType': returnType, 'items': items.map((i) => i.toMap()).toList()}; factory PurchaseReturn.fromMap(Map<String, dynamic> map) => PurchaseReturn(id: map['id'], billNo: map['billNo'], date: DateTime.parse(map['date']), distributorName: map['distributorName'], totalAmount: (map['totalAmount'] ?? 0.0).toDouble(), status: map['status'] ?? "Active", returnType: map['returnType'] ?? "Sellable", items: (map['items'] as List).map((i) => PurchaseItem.fromMap(i)).toList()); }
