...
class Medicine {
  String id, name, packing, manufacturer, hsnCode; double gst, mrp, purRate, rateA, rateB, rateC; int stock;
...
class Party {
  String id, name, address, city, state, route, phone, email, dl, gst, rateType; double openingBalance, specialDiscount;
...
class BillItem {
  String id, medicineID, name, packing, batch, exp, hsn; int srNo; double mrp, qty, rate, discountPercent, discountRupees, gstRate, cgst, sgst, igst, total;
...
class Sale {
  ...
  factory Sale.fromMap(Map<String, dynamic> map) => Sale(id: map['id'], billNo: map['billNo'], date: DateTime.parse(map['date']), partyName: map['partyName'], partyGstin: map['partyGstin'] ?? "N/A", partyState: map['partyState'] ?? "Rajasthan", paymentMode: map['paymentMode'] ?? "CASH", totalAmount: (map['totalAmount'] ?? 0).toDouble(), status: map['status'] ?? "Active", invoiceType: map['invoiceType'] ?? "B2C", transporterName: map['transporterName'] ?? "", transporterId: map['transporterId'] ?? "", vehicleNo: map['vehicleNo'] ?? "", items: (map['items'] as List).map((i) => BillItem.fromMap(i)).toList());
}
...
class Purchase {
  String id, internalNo, billNo, distributorName, paymentMode, gstStatus; DateTime date; List<PurchaseItem> items; double totalAmount;
