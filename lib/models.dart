...
// ==========================================
// 1. BATCH INFORMATION (Medicine Batches)
// ==========================================
class BatchInfo {
...
// ==========================================
// 3. PRODUCT MASTER (Inventory)
// ==========================================
class Medicine {
  String id, name, packing, manufacturer, hsnCode; 
  double gst, mrp, purRate, rateA, rateB, rateC, stock; // Stock is double for 5.5 tabs etc.
...
// ==========================================
// 4. LEDGER / ACCOUNT MASTER (Advanced)
// ==========================================
class Party {
  ...
  String dlExpiry; // Pharma DL Expiry
  double creditLimit;
  String creditControlMode; // 'Soft' (Warning) or 'Hard' (Block)
...
// ==========================================
// 5. BILL ITEM (Line Item for Sales)
// ==========================================
class BillItem {
...
class Sale {
  ...
  Sale({required this.id, required this.billNo, required this.date, required this.partyName, required this.partyGstin, required this.partyState, required this.partyAddress, required this.partyDl, required this.partyEmail, required this.items, required this.totalAmount, required this.paymentMode, this.status = "Active", this.invoiceType = "B2C", this.transporterName = "", this.transporterId = "", this.vehicleNo = "", this.amountPaid = 0.0});

  double get pendingAmount => totalAmount - amountPaid;

  Map<String, dynamic> toMap() => {'id': id, 'billNo': billNo, 'date': date.toIso8601String(), 'partyName': partyName, 'partyGstin': partyGstin, 'partyState': partyState, 'partyAddress': partyAddress, 'partyDl': partyDl, 'partyEmail': partyEmail, 'paymentMode': paymentMode, 'totalAmount': totalAmount, 'amountPaid': amountPaid, 'status': status, 'invoiceType': invoiceType, 'transporterName': transporterName, 'transporterId': transporterId, 'vehicleNo': vehicleNo, 'items': items.map((i) => i.toMap()).toList()};
...
// ==========================================
// 7. PURCHASE ITEM (Line Item for Purchases)
// ==========================================
class PurchaseItem {
  String id, medicineID, name, packing, batch, exp, hsn; 
  int srNo; 
...
class Purchase {
  ...
  Purchase({required this.id, required this.internalNo, required this.billNo, required this.date, required this.entryDate, required this.distributorName, required this.items, required this.totalAmount, required this.paymentMode, this.gstStatus = "Pending", this.amountPaid = 0.0});

  double get pendingAmount => totalAmount - amountPaid;

  Map<String, dynamic> toMap() => {'id': id, 'internalNo': internalNo, 'billNo': billNo, 'date': date.toIso8601String(), 'entryDate': entryDate.toIso8601String(), 'distributorName': distributorName, 'paymentMode': paymentMode, 'totalAmount': totalAmount, 'amountPaid': amountPaid, 'gstStatus': gstStatus, 'items': items.map((i) => i.toMap()).toList()};
...
// ==========================================
// 9. ACCOUNTING VOUCHER (Receipt / Payment)
// ==========================================
class Voucher {
  String id, type; // 'Receipt' (Cash In) or 'Payment' (Cash Out)
  DateTime date;
  String partyId, partyName, paymentMode, narration, refBillId, refBillNo;
  double amount;
  bool isAgainstBill;

  Voucher({required this.id, required this.type, required this.date, required this.partyId, required this.partyName, required this.amount, required this.paymentMode, this.narration = "", this.isAgainstBill = false, this.refBillId = "", this.refBillNo = ""});

  Map<String, dynamic> toMap() => {'id': id, 'type': type, 'date': date.toIso8601String(), 'partyId': partyId, 'partyName': partyName, 'amount': amount, 'paymentMode': paymentMode, 'narration': narration, 'isAgainstBill': isAgainstBill, 'refBillId': refBillId, 'refBillNo': refBillNo};
  factory Voucher.fromMap(Map<String, dynamic> map) => Voucher(id: map['id'], type: map['type'], date: DateTime.parse(map['date']), partyId: map['partyId'], partyName: map['partyName'], amount: (map['amount'] ?? 0.0).toDouble(), paymentMode: map['paymentMode'] ?? "Cash", narration: map['narration'] ?? "", isAgainstBill: map['isAgainstBill'] ?? false, refBillId: map['refBillId'] ?? "", refBillNo: map['refBillNo'] ?? "");
}
