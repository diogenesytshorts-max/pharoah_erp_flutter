// FILE: lib/models.dart (Replacement Code)
...
class Medicine {
  ...
  // NAYA: Aadhaar Card logic (ID overlap fix karne ke liye)
...
class Party {
  ...
  Party({
    ...
    this.transport = "", 
    this.priceLevel = "A", 
    this.defaultSeriesId = "", // NAYA: Linked Billing Series
    this.opBal = 0.0, 
  ...
  Map<String, dynamic> toMap() => {
    ...
    'pan': pan, 'transport': transport, 'priceLevel': priceLevel, 'defaultSeriesId': defaultSeriesId,
  ...
  factory Party.fromMap(Map<String, dynamic> map) => Party(
    id: map['id'] ?? "", 
    name: map['name'] ?? "", 
    group: map['group'] ?? "Sundry Debtors", 
    phone: map['phone'] ?? "", 
    email: map['email'] ?? "", 
    address: map['address'] ?? "", 
    city: map['city'] ?? "", 
    ...
    priceLevel: map['priceLevel'] ?? "A", 
    defaultSeriesId: map['defaultSeriesId'] ?? "",
    opBal: (map['opBal'] ?? 0.0).toDouble(), 
...
class BillItem {
  ...
  factory BillItem.fromMap(Map<String, dynamic> map) => BillItem(
    id: map['id'] ?? "", srNo: map['srNo'] ?? 0, medicineID: map['medicineID'] ?? "", name: map['name'] ?? "", 
    packing: map['packing'] ?? "", batch: map['batch'] ?? "", exp: map['exp'] ?? "", hsn: map['hsn'] ?? "", 
    mrp: (map['mrp'] ?? 0.0).toDouble(), qty: (map['qty'] ?? 0.0).toDouble(), freeQty: (map['freeQty'] ?? 0.0).toDouble(), 
    rate: (map['rate'] ?? 0.0).toDouble(), gstRate: (map['gstRate'] ?? 0.0).toDouble(), cgst: (map['cgst'] ?? 0.0).toDouble(), 
    sgst: (map['sgst'] ?? 0.0).toDouble(), igst: (map['igst'] ?? 0.0).toDouble(), total: (map['total'] ?? 0.0).toDouble(), 
    discountRupees: (map['discountRupees'] ?? 0.0).toDouble(), sourceChallanNo: map['sourceChallanNo'] ?? ""
  );
}
...
class PurchaseItem {
  ...
  factory PurchaseItem.fromMap(Map<String, dynamic> map) => PurchaseItem(
    ...
    packing: map['packing'] ?? "", batch: map['batch'] ?? "", exp: map['exp'] ?? "", hsn: map['hsn'] ?? "", 
    mrp: (map['mrp'] ?? 0.0).toDouble(), qty: (map['qty'] ?? 0.0).toDouble(), freeQty: (map['freeQty'] ?? 0.0).toDouble(), 
    purchaseRate: (map['purchaseRate'] ?? 0.0).toDouble(), gstRate: (map['gstRate'] ?? 0.0).toDouble(), total: (map['total'] ?? 0.0).toDouble(), 
    rateA: (map['rateA'] ?? 0.0).toDouble(), rateB: (map['rateB'] ?? 0.0).toDouble(), rateC: (map['rateC'] ?? 0.0).toDouble()
  );
}
...
class Sale { 
  ...
  Sale({required this.id, required this.billNo, required this.date, required this.partyName, required this.partyGstin, required this.partyState, required this.items, required this.totalAmount, required this.paymentMode, this.status = "Active", this.invoiceType = "B2C", this.transporterName = "", this.transporterId = "", this.vehicleNo = "", this.salesmanName = ""});

  Map<String, dynamic> toMap() => {'id': id, 'billNo': billNo, 'date': date.toIso8601String(), 'partyName': partyName, 'partyGstin': partyGstin, 'partyState': partyState, 'paymentMode': paymentMode, 'totalAmount': totalAmount, 'status': status, 'invoiceType': invoiceType, 'transporterName': transporterName, 'transporterId': transporterId, 'vehicleNo': vehicleNo, 'salesmanName': salesmanName, 'items': items.map((i) => i.toMap()).toList()};
...
class BatchInfo { 
  ...
  double mrp, rate, qty, openingQty, adjustmentQty, breakageQty; bool isShell;
...
class Voucher { 
  ...
  factory Voucher.fromMap(Map<String, dynamic> map) => Voucher(id: map['id'] ?? "", type: map['type'] ?? "", date: DateTime.parse(map['date'] ?? DateTime.now().toIso8601String()), partyId: map['partyId'] ?? "", partyName: map['partyName'] ?? "", amount: (map['amount'] ?? 0.0).toDouble(), paymentMode: map['paymentMode'] ?? "Cash", narration: map['narration'] ?? ""); 
}
