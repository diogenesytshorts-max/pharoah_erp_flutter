...
class _ItemLedgerDetailViewState extends State<ItemLedgerDetailView> {
  ...
  @override Widget build(BuildContext context) {
    ...
    for (var p in ph.purchases) { for (var it in p.items) { if (it.medicineID == widget.medicine.id) { entries.add(LedgerEntry(date: p.date, type: "IN", refNo: p.billNo, party: p.distributorName, batch: it.batch, qty: it.qty + it.freeQty, rate: it.purchaseRate)); totalIn += (it.qty + it.freeQty); uniqueBatches.add(it.batch); } } }
    for (var s in ph.sales.where((s) => s.status == "Active")) { for (var it in s.items) { if (it.medicineID == widget.medicine.id) { entries.add(LedgerEntry(date: s.date, type: "OUT", refNo: s.billNo, party: s.partyName, batch: it.batch, qty: it.qty, rate: it.rate)); totalOut += it.qty; uniqueBatches.add(it.batch); } } }
    ...
    return Scaffold(
      ...
      body: Column(children: [
        Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: Colors.white, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 5)]), child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [_sumBox("TOTAL IN", totalIn.toInt().toString(), Colors.green), _sumBox("TOTAL OUT", totalOut.toInt().toString(), Colors.red), _sumBox("BALANCE", widget.medicine.stock.toString(), Colors.blue.shade800)])),
        ...
        Expanded(child: displayEntries.isEmpty ? const Center(child: Text("No transaction history found.")) : ListView.builder(padding: const EdgeInsets.all(10), itemCount: displayEntries.length, itemBuilder: (c, i) {
          ...
          return Card(margin: const EdgeInsets.only(bottom: 8), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10), side: BorderSide(color: isIn ? Colors.green.shade200 : Colors.red.shade200, width: 1)), child: ListTile(
            ...
            trailing: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.end, children: [Text(isIn ? "+ ${e.qty.toInt()}" : "- ${e.qty.toInt()}", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 18, color: isIn ? Colors.green : Colors.red)), Text("Qty", style: TextStyle(fontSize: 10, color: Colors.grey.shade600))]),
          ));
        }))
      ]),
    );
