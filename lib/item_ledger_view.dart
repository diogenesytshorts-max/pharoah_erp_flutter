...
import 'models.dart';
import 'bill_view_only.dart'; // FIXED: Crucial Import
import 'purchase/purchase_view_only.dart'; // FIXED: Crucial Import
      ...
      body: Column(children: [
        ...
        Expanded(child: displayHistory.isEmpty ? const Center(child: Text("No transactions found")) : ListView.builder(padding: const EdgeInsets.all(10), itemCount: displayHistory.length, itemBuilder: (c, i) {
              ...
              return Card(child: ListTile(onTap: () {
                    if (isIn) { Navigator.push(context, MaterialPageRoute(builder: (c) => PurchaseViewOnly(purchase: h['bill']))); } 
                    else {
                      final partyObj = ph.parties.firstWhere((p) => p.name == h['party'], orElse: () => Party(id: "", name: h['party']));
                      Navigator.push(context, MaterialPageRoute(builder: (c) => BillViewOnly(sale: h['bill'], party: partyObj)));
                    }
