...
class _PurchaseBillingViewState extends State<PurchaseBillingView> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [TextButton(onPressed: items.isEmpty ? null : () => _save(ph), child: const Text("SAVE", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)))]
      ),
      body: Column(children: [
        if (selectedMed == null)
          Padding(padding: const EdgeInsets.all(10), child: TextField(decoration: const InputDecoration(hintText: "Search Product for Purchase...", border: OutlineInputBorder()), onChanged: (v) => setState(() => search = v))),
        if (selectedMed != null)
        ...
        Expanded(child: ListView.builder(itemCount: items.length, itemBuilder: (c, i) => ListTile(
          ...
          subtitle: Text("Qty: ${items[i].qty} + ${items[i].freeQty} | Rate: ${items[i].purchaseRate}"),
          trailing: Text("₹${items[i].total.toStringAsFixed(2)}"),
        ))),
        Container(padding: const EdgeInsets.all(15), color: Colors.orange[50], child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Items: ${items.length}"), Text("TOTAL: ₹${totalAmt.toStringAsFixed(2)}", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.deepOrange))]))
      ]),
    );
  }
  ...
  void _save(PharoahManager ph) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Purchase Bill Saved!")));
  }
}
...
class _PurchaseItemEntryFormState extends State<PurchaseItemEntryForm> {
  final bC = TextEditingController(); final eC = TextEditingController(); final mC = TextEditingController(); 
  final rC = TextEditingController(); final qC = TextEditingController(text: "1"); final fC = TextEditingController(text: "0"); 
  ...
  @override Widget build(BuildContext context) {
    return Container(padding: const EdgeInsets.all(15), color: Colors.white, child: Column(children: [
      ...
      Row(children: [
        Expanded(child: TextField(controller: mC, decoration: const InputDecoration(labelText: "MRP"))),
      ...
      ElevatedButton(style: ElevatedButton.styleFrom(minimumSize: const Size(double.infinity, 45), backgroundColor: Colors.orange), onPressed: () {
        double rt = double.tryParse(rC.text) ?? 0; double qt = double.tryParse(qC.text) ?? 0; double gst = double.tryParse(gC.text) ?? 0;
        widget.onAdd(PurchaseItem(id: DateTime.now().toString(), srNo: widget.srNo, medicineID: widget.med.id, name: widget.med.name, packing: widget.med.packing, batch: bC.text.toUpperCase(), exp: eC.text, mrp: double.tryParse(mC.text) ?? 0, qty: qt, freeQty: double.tryParse(fC.text) ?? 0, purchaseRate: rt, gstRate: gst, total: (rt * qt) * (1 + gst / 100)));
