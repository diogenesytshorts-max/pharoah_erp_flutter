...
class _PurchaseBillingViewState extends State<PurchaseBillingView> {
      ...
      body: Column(children: [
        ...
        Expanded(child: ListView.separated(
          ...
          itemBuilder: (c, i) => ListTile(
            ...
            subtitle: Text("Batch: ${items[i].batch} | Exp: ${items[i].exp} | Qty: ${items[i].qty.toInt()}"),
  ...
  Widget _buildSearchList(PharoahManager ph) {
    return Container(
...
class _PurchaseItemFormState extends State<PurchaseItemForm> {
  ...
  @override Widget build(BuildContext context) {
    return Container(padding: const EdgeInsets.all(15), color: Colors.orange.shade50, child: Column(children: [
      ...
      Row(children: [Expanded(child: _field(mC, "MRP", onCh: (v) => _calcRateC())), const SizedBox(width: 5), Expanded(child: _field(pRC, "Pur. Rate")), const SizedBox(width: 5), Expanded(child: _field(qC, "Qty"))]),
      const SizedBox(height: 10),
      Row(children: [Expanded(child: _field(rAC, "Rate A")), const SizedBox(width: 5), Expanded(child: _field(rBC, "Rate B")), const SizedBox(width: 5), 
        Expanded(child: GestureDetector(onTap: () => setState(() => showDisc = !showDisc), child: _field(rCC, "Rate C", en: false)))
      ]),
  ...
  Widget _field(ctrl, l, {bool en = true, Function(String)? onCh}) => TextField(controller: ctrl, enabled: en, onChanged: onCh, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: InputDecoration(labelText: l, border: const OutlineInputBorder(), contentPadding: const EdgeInsets.all(8), filled: !en, fillColor: Colors.white));
}
