        ...
        Column(children: [
          ...
          Expanded(child: ListView.separated(
            itemCount: items.length, separatorBuilder: (c, i) => const Divider(), itemBuilder: (c, i) => ListTile(
              ...
              trailing: Text("â¹${items[i].total.toStringAsFixed(2)}"),
          ...
          Container(padding: const EdgeInsets.all(15), color: Colors.orange.shade50, child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("TOTAL ITEMS: ${items.length}"), Text("NET TOTAL: â¹${totalAmt.toStringAsFixed(2)}", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.deepOrange))]))
        ]),
      ]),
    );
  }
  ...
  @override Widget build(BuildContext context) {
    return Container(padding: const EdgeInsets.all(15), color: Colors.orange.shade50, child: Column(children: [
      ...
      Row(children: [
        ...
        const SizedBox(width: 5), Expanded(child: _field(gC, "GST%", onCh: (v) => _calcRateC())),
      ]),
      const SizedBox(height: 10),
      Row(children: [
        Expanded(child: _field(mC, "MRP", onCh: (v) => _calcRateC())),
        const SizedBox(width: 5), Expanded(child: _field(pRC, "Pur. Rate")), 
