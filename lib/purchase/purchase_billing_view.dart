  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(backgroundColor: Colors.orange.shade800, foregroundColor: Colors.white, title: Text(widget.distributor.name),
        actions: [TextButton(onPressed: items.isEmpty ? null : () => _handleSave(ph), child: const Text("SAVE", style: TextStyle(color: Colors.white)))],
      ),
      body: Column(
        children: [
          if (selectedMed == null)
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (widget.batchHistory.isNotEmpty) ...[
            ...
            SizedBox(height: 40, child: ListView(scrollDirection: Axis.horizontal, children: widget.batchHistory.map((b) => Padding(padding: const EdgeInsets.only(right: 5), child: ActionChip(label: Text(b.batch), onPressed: () {
              setState(() { bC.text = b.batch; eC.text = b.exp; mC.text = b.mrp.toString(); pRC.text = b.rate.toString(); originalExp = b.exp; });
            }))).toList())),
          ...
          ElevatedButton(onPressed: () async {
            if (originalExp != null && originalExp != eC.text) {
               bool confirm = await showDialog(context: context, builder: (c) => AlertDialog(title: const Text("Update Expiry?"), content: const Text("Batch expiry is changing. Continue?"), actions: [TextButton(onPressed: () => Navigator.pop(c, false), child: const Text("NO")), TextButton(onPressed: () => Navigator.pop(c, true), child: const Text("YES"))])) ?? false;
               if (!confirm) return;
            }
            double pr = double.tryParse(pRC.text) ?? 0; double qt = double.tryParse(qC.text) ?? 0;
            widget.onAdd(PurchaseItem(id: DateTime.now().toString(), srNo: widget.srNo, medicineID: widget.med.id, name: widget.med.name, packing: widget.med.packing, batch: bC.text.toUpperCase(), exp: eC.text, hsn: widget.med.hsnCode, mrp: double.tryParse(mC.text) ?? 0, qty: qt, purchaseRate: pr, gstRate: double.tryParse(gC.text) ?? 0, total: (pr * qt) * (1 + (double.tryParse(gC.text) ?? 0)/100)));
