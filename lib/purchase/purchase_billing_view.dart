...
class PurchaseBillingView extends StatefulWidget {
  ...
  final DateTime billDate, entryDate; // Dono dates yahan aengi
  final String mode;
...
class _PurchaseBillingViewState extends State<PurchaseBillingView> {
  ...
  void _handleSave(PharoahManager ph) async { 
    ...
    // Passing both dates to finalizePurchase
    ph.finalizePurchase(
      internalNo: widget.internalNo, 
      billNo: widget.distBillNo, 
      date: widget.billDate, 
      entryDate: widget.entryDate, // Naya parameter pass kiya
    ...
    if (mounted) { 
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("✅ Purchase Saved/Updated!"), backgroundColor: Colors.orange)); 
...
class _PurchaseItemEntryFormState extends State<PurchaseItemEntryForm> {
  ...
  @override Widget build(BuildContext context) {
    return Container(padding: const EdgeInsets.all(15), color: Colors.orange.shade50, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(children: [Expanded(child: _field(mC, "MRP", onCh: (v) => _calcRateC())), const SizedBox(width: 5), Expanded(child: _field(pRC, "Pur Rate")), const SizedBox(width: 5), Expanded(child: _field(qC, "Qty", isNum: true)), const SizedBox(width: 5), Expanded(child: _field(fC, "Free", isNum: true))]),
      const SizedBox(height: 10),
