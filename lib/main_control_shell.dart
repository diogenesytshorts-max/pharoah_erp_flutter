...
class _MainControlShellState extends State<MainControlShell> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          if (ph.activeModule != "HOME") IconButton(icon: const Icon(Icons.grid_view_rounded), onPressed: () => ph.updateModule("HOME")),
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        onTap: (i) { if(i==0) ph.updateModule("HOME"); },
        items: const [
  ...
  Widget _buildLiveKpiStrip(PharoahManager ph) {
    ...
    double todaySales = ph.sales.where((s) => s.status == "Active" && s.date.day == now.day).fold(0.0, (sum, s) => sum + s.totalAmount);
    return Row(children: [
  ...
  Widget _buildAlertsBar() {
    return Container(
      ...
      child: const Row(children: [
          ...
          SizedBox(width: 10),
          Text("System Alert: Check Inventory!", style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold, fontSize: 11)),
      ]),
    );
  }
