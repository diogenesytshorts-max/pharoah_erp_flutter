...
class PharoahManager with ChangeNotifier {
  ...
  Future<void> loadAllData() async {
    final path = await _localPath;
    try {
      final mf = File('$path/meds_$currentFY.json'); medicines = mf.existsSync() ? (jsonDecode(mf.readAsStringSync()) as List).map((e) => Medicine.fromMap(e)).toList() : DemoData.getMedicines();
      final pf = File('$path/parts_$currentFY.json'); parties = pf.existsSync() ? (jsonDecode(pf.readAsStringSync()) as List).map((e) => Party.fromMap(e)).toList() : [DemoData.getDemoParty(), Party(id: 'cash', name: "CASH")];
  ...
  void finalizeSale({required String billNo, required DateTime date, required Party party, required List<BillItem> items, required double total, required String mode}) {
    sales.add(Sale(id: DateTime.now().toString(), billNo: billNo, date: date, partyName: party.name, partyGstin: party.gst, partyState: party.state, partyAddress: party.address, partyDl: party.dl, items: items, totalAmount: total, paymentMode: mode, invoiceType: party.isB2B ? "B2B" : "B2C"));
    for (var it in items) {
