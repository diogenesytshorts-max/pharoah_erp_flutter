...
class PharoahManager with ChangeNotifier {
  ...
  Future<void> finalizeBatchSales(List<Sale> batch) async {
    ...
    for (var s in batch) {
      if (s.linkedChallanIds.isNotEmpty) {
        for (var id in s.linkedChallanIds) {
  ...
  void finalizeSaleChallan({required String challanNo, required DateTime date, required Party party, required List<BillItem> items, required double total, String remarks = ""}) async { 
    saleChallans.add(SaleChallan(id: DateTime.now().toString(), billNo: challanNo, partyId: party.id, date: date, partyName: party.name, partyGstin: party.gst, partyState: party.state, items: items, totalAmount: total, remarks: remarks)); 
    save(); 
  }
  ...
  void finalizePurchaseChallan({required String challanNo, required String internalNo, required DateTime date, required Party party, required List<PurchaseItem> items, required double total, String remarks = ""}) async { 
    purchaseChallans.add(PurchaseChallan(id: DateTime.now().toString(), internalNo: internalNo, billNo: challanNo, partyId: party.id, date: date, distributorName: party.name, items: items, totalAmount: total, remarks: remarks)); 
