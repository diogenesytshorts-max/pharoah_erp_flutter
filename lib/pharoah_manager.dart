// FILE: lib/pharoah_manager.dart (100% Complete & Re-verified)
...
import 'models.dart';
import 'administration/system_user_model.dart'; // SECURITY MODEL
...
class PharoahManager with ChangeNotifier {
  ...
  // --- ADVANCED MODULE LISTS ---
  List<ShortageItem> shortages = []; 
  List<Bank> banks = []; 
  List<Salesman> salesmen = [];
  List<ChequeEntry> cheques = []; 
  List<SystemUser> systemUsers = []; // STAFF USERS
  ...
  SystemUser? loggedInStaff; // Null = Admin, Not Null = Staff Login
  ...
  // ===========================================================================
  // 4. INTELLIGENCE (SHORTAGE ENGINE)
  // ===========================================================================
  double calculateAvgMonthlySale(String medId) {
    ...
    for (var s in sales.where((x) => x.status == "Active" && x.date.isAfter(ninetyDaysAgo))) {
      for (var it in s.items.where((i) => i.medicineID == medId)) { totalQty += (it.qty + it.freeQty); }
    }
    return totalQty / 3;
  }
  ...
  void runAutoShortageScan() {
    for (var med in medicines) {
      ...
      if (required > 0) {
        ...
        if (idx != -1) { shortages[idx].qtyRequired = required; shortages[idx].currentStock = med.stock; } 
        else { shortages.add(ShortageItem(id: DateTime.now().toString() + med.id, medicineId: med.id, medicineName: med.name, companyName: med.companyId, source: "Auto", qtyRequired: required, currentStock: med.stock, date: DateTime.now())); }
      }
    }
    save();
  }
  ...
  // ===========================================================================
  // 5. TRANSACTION ACTIONS (WITH SALESMAN AUTO-TAGGING)
  // ===========================================================================
  ...
  Future<void> startNewFinancialYear(String nextFY) async {
