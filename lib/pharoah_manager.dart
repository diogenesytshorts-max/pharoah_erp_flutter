...
import 'dart:convert';
import 'dart:io';
import 'dart:async'; // NAYA: Timer ke liye
import 'dart:typed_data';
import 'package:flutter/material.dart';
import 'package:path_provider/path_provider.dart';
import 'package:local_auth/local_auth.dart'; // NAYA: Hardware calls
import 'package:flutter_secure_storage/flutter_secure_storage.dart'; // NAYA: Secure Storage
...
class PharoahManager with ChangeNotifier {
  ...
  // --- 🛡️ NEW SECURITY & AUTO-LOCK STATE (Sahi Jagah) ---
  ...
  List<ModuleAction> get billingActions => [
    ...
    ModuleAction(title: "STITCHER", icon: Icons.auto_fix_high_rounded, color: Colors.teal, navModule: "GO_STITCHER_WIZARD"),
  ...
  List<ModuleAction> get challanActions => [
    ...
    ModuleAction(title: "Pur Challan", icon: Icons.inventory_2, color: Colors.amber.shade800, navModule: "GO_CHALLAN_PUR"),
    ModuleAction(title: "Sale Reg", icon: Icons.format_list_bulleted_rounded, color: Colors.indigo, navModule: "GO_CHALLAN_SALE_REG"),
    ModuleAction(title: "Pur Reg", icon: Icons.history_edu_rounded, color: Colors.amber.shade900, navModule: "GO_CHALLAN_PUR_REG"),
  ];
  ...
  void authenticateAdmin(bool status) { 
    ...
    if (status) {
      isAppLocked = false;
      resetInactivityTimer(); // Login par timer shuru
    }
    notifyListeners(); 
  }
  ...
  Future<bool> authenticateBiometric() async {
    try {
      ...
      bool didAuthenticate = await _auth.authenticate(
        localizedReason: 'Scan fingerprint to unlock Pharoah ERP',
  ...
  void finalizePurchase({required String internalNo, required String billNo, required DateTime date, DateTime? entryDate, required Party party, required List<PurchaseItem> items, required double total, required String mode}) { 
    purchases.add(Purchase(id: DateTime.now().toString(), internalNo: internalNo, billNo: billNo, partyId: party.id, date: date, entryDate: entryDate ?? DateTime.now(), distributorName: party.name, items: items, totalAmount: total, paymentMode: mode)); 
