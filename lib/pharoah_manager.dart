...
class PharoahManager with ChangeNotifier {
  ...
  // ===========================================================================
  // 1. BATCH REGISTRATION (PRESERVES EXACT CASE: aQ14256BBc)
  // ===========================================================================
  void registerBatchActivity({
    ...
    // MATCHING: Exact case matching for mirroring
    ...
    if (existingIdx != -1) {
      ...
      history.add(BatchInfo(
        batch: batchNo.trim(), // Case preserved
        exp: exp,
        packing: packing,
        mrp: mrp,
        rate: rate,
  ...
  // ===========================================================================
  // 2. TRANSACTION FINALIZATION (C2C MIRROR READY)
  // ===========================================================================
  ...
  void finalizePurchase({
    ...
    purchases.add(Purchase(
      ...
      partyId: party.id, date: date, entryDate: entryDate ?? DateTime.now(), 
      distributorName: party.name, items: items, totalAmount: total, 
      paymentMode: mode, linkedChallanIds: linkedChallanIds ?? [], 
      sourceTag: sourceTag
    )); 
  ...
  Future<void> finalizeBatchSales(List<Sale> batch) async { 
    ...
    for (var s in batch) { 
      if (s.linkedChallanIds.isNotEmpty) { 
        for (var id in s.linkedChallanIds) { int i = saleChallans.indexWhere((c) => c.id == id); if (i != -1) saleChallans[i].status = "Billed"; } 
      } 
    } 
    if (batch.isNotEmpty && activeCompany != null) { 
      String l = batch.last.billNo; 
      String p = l.split(RegExp(r'\d')).first; 
      await PharoahNumberingEngine.updateSeriesCounter(type: "SALE", companyID: activeCompany!.id, usedNumber: l, prefix: p); 
    } 
    await save(); 
    InventoryLogicCenter.rebuildAllInventory(medicines: medicines, batchHistory: batchHistory, purchases: purchases, sales: sales); 
    notifyListeners(); 
  ...
  Future<void> finalizeBatchPurchases(List<Purchase> batch) async { 
    ...
    for (var p in batch) { 
      if (p.linkedChallanIds.isNotEmpty) { 
        for (var id in p.linkedChallanIds) { int i = purchaseChallans.indexWhere((c) => c.id == id); if (i != -1) purchaseChallans[i].status = "Billed"; } 
      } 
    } 
    await save(); 
    InventoryLogicCenter.rebuildAllInventory(medicines: medicines, batchHistory: batchHistory, purchases: purchases, sales: sales); 
