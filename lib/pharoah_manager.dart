...
class PharoahManager with ChangeNotifier {
  ...
  // Initial Setup: FY read karo aur data load karo
  ...
  Future<void> loadAllData() async {
    ...
    try {
      ...
      if (mf.existsSync()) {
        ...
        medicines = DemoData.getMedicines(); // Load demo if new
      }
      ...
      if (sf.existsSync()) {
        ...
        sales = d.map((e) => Sale(
          ...
          paymentMode: e['paymentMode'], status: e['status'] ?? "Active", 
          date: DateTime.parse(e['date']), totalAmount: (e['totalAmount']??0).toDouble(), 
  ...
  // --- FINAL SALE: Stock Decrease + Batch History Update ---
  void finalizeSale({required String billNo, required DateTime date, required Party party, required List<BillItem> items, required double total, required String mode}) {
    addLog("SALE", "Bill $billNo issued to ${party.name}");
    ...
    for (var item in items) {
      ...
      if (idx != -1) {
        ...
        // Save this batch info for future suggestions
  ...
  // --- FINAL PURCHASE: Stock Increase + Master Rates Update ---
  void finalizePurchase({required String internalNo, required String billNo, required DateTime date, required Party party, required List<PurchaseItem> items, required double total, required String mode}) {
    ...
    for (var item in items) {
      int idx = medicines.indexWhere((m) => m.id == item.medicineID);
      if (idx != -1) {
        medicines[idx].stock += (item.qty + item.freeQty).toInt(); // Increase Stock
        medicines[idx].mrp = item.mrp;
        medicines[idx].gst = item.gstRate;
        
        // --- Valuation & Price Master Updates ---
        medicines[idx].purRate = item.purchaseRate; // Latest Purchase Cost
  ...
  // Helper for Batch History Suggestion
  ...
  // --- DELETE BILL: Full Removal + Stock Reverse ---
  void deleteBill(String id) {
    int i = sales.indexWhere((s) => s.id == id);
    if (i != -1) {
      addLog("DELETE", "Bill ${sales[i].billNo} deleted permanently.");
  ...
  // --- CANCEL BILL: Keep Record but Zero Amount + Stock Reverse ---
  void cancelBill(String id) {
    int i = sales.indexWhere((s) => s.id == id);
    if (i != -1 && sales[i].status != "Cancelled") {
      addLog("CANCEL", "Bill ${sales[i].billNo} marked as Cancelled.");
      for (var it in sales[i].items) {
        int mi = medicines.indexWhere((m) => m.id == it.medicineID);
        if (mi != -1) medicines[mi].stock += it.qty.toInt(); // Reverse Stock
      }
  ...
  // Helper to add new items on the fly
