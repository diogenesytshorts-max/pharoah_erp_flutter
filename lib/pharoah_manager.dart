...
class PharoahManager with ChangeNotifier {
  ...
  Future<void> setupNewCompanyEnvironment(CompanyProfile profile, String initialFY) async {
    ...
    final dir = Directory('${(await getApplicationDocumentsDirectory()).path}/Pharoah_Data/${profile.id}/${profile.businessType}/$initialFY');
  ...
  Future<void> loadAllData() async {
    ...
    if (bats != null) { batchHistory.clear(); (bats as Map).forEach((k, v) => batchHistory[k] = (v as List).map((b) => BatchInfo.fromMap(b)).toList()); }
  ...
  void updateBatchMetadata({required String medId, required String batchNo, required String newExp, required double newMrp, required double newRate}) { if (batchHistory.containsKey(medId)) { var b = batchHistory[medId]!.firstWhere((x) => x.batch == batchNo); b.exp = newExp; b.mrp = newMrp; b.rate = newRate; save().then((_) => loadAllData()); } }
}
