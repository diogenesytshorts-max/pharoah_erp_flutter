...
class _ImportReviewScreenState extends State<ImportReviewScreen> {
  ...
  void _processUniversalLogic() {
    ...
    for (int i = 1; i < data.length; i++) {
      ...
      double csvTotal = double.tryParse(row[34].toString()) ?? 0.0; // INDEX 34

      // AUTO CALCULATION LOGIC (To avoid manual register updates)
      ...
      reviewedItems.add({
        ...
        'match': match, 'isSelected': match != null, 'status': match == null ? 'new' : 'exact', 'isFixed': false,
        'taxable': taxable, 'totalTax': taxAmt,
      });
    }
  ...
  // ===========================================================================
  // 2. BULK ACTION: SEQUENTIAL ID AUTO-CREATE (PH- SERIES)
  // ===========================================================================
  void _autoResolveAllNewProducts(PharoahManager ph) async {
    ...
    for (int idx in newIndices) {
      ...
      final newMed = Medicine(id: DateTime.now().millisecondsSinceEpoch.toString() + idx.toString(), systemId: generatedSysId, name: it['name'], packing: it['pack'], hsnCode: it['hsn'], gst: it['gstPer'], mrp: it['mrp'], purRate: it['purRate'], rateA: it['rate'], drugForm: it['form'], isNarcotic: it['isNaco'], isScheduleH1: it['isH1'], companyId: ph.getOrCreateCompany(it['mfg']), saltId: ph.getOrCreateSalt(it['salt']));
      ph.addMedicine(newMed);
      it['match'] = newMed; it['status'] = 'exact'; it['isSelected'] = true; currentNum++;
    }
    await ph.save(); setState(() => isLoading = false);
  }

  // ===========================================================================
  // 3. UI HELPERS (PARTY PICKER & ITEM LINKING)
  // ===========================================================================
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: const Color(0xFF0F172A), shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(25))), builder: (c) {
      String s = ""; return StatefulBuilder(builder: (context, setPickerState) {
        ...
        return Container(height: MediaQuery.of(context).size.height * 0.8, padding: const EdgeInsets.all(20), child: Column(children: [
          ...
          Expanded(child: ListView.builder(itemCount: list.length, itemBuilder: (c, i) => ListTile(title: Text(list[i].name, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), subtitle: Text(list[i].city, style: const TextStyle(color: Colors.white38, fontSize: 11)), onTap: () { setState(() => matchedParty = list[i]); Navigator.pop(context); })))
        ]));
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Expanded(child: ListView.builder(padding: const EdgeInsets.all(12), itemCount: reviewedItems.length, itemBuilder: (c, i) => _buildItemRow(i))),
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const Text("TAP TO VERIFY & MANAGE", style: TextStyle(color: Colors.blueAccent, fontSize: 9, fontWeight: FontWeight.bold)),
          ])),
    ...
    return Container(
      ...
      child: Column(children: [
        ListTile(
          ...
          subtitle: Wrap(spacing: 5, children: [ _badge("PK: ${it['pack']}", Colors.blueGrey), _badge("BT: ${it['batch']}", Colors.indigo), _badge("EXP: ${it['exp']}", Colors.deepPurple), ]),
  ...
  Widget _buildAnalyticsFooter(PharoahManager ph) {
    ...
    return Container(padding: const EdgeInsets.all(20), decoration: const BoxDecoration(color: Color(0xFF1E293B), borderRadius: BorderRadius.vertical(top: Radius.circular(25))), child: Column(mainAxisSize: MainAxisSize.min, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [ _footStat("NET SYSTEM", sysTotal), _footStat("NET CSV", csvTotal), _footStat("DIFF", diff, color: diff.abs() > 0.1 ? Colors.redAccent : Colors.greenAccent), ]),
      const Divider(color: Colors.white10, height: 25),
      SizedBox(width: double.infinity, height: 55, child: ElevatedButton.icon(onPressed: () => _handleFinalImport(ph), icon: const Icon(Icons.cloud_done), label: const Text("FINALIZE & MIRROR DATA", style: TextStyle(fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(backgroundColor: Colors.blueAccent, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)))))
    ]));
  }
  ...
  void _handleFinalImport(PharoahManager ph) async {
    ...
    setState(() => isLoading = true);
    double finalGross = reviewedItems.where((e)=>e['isSelected']).fold(0.0, (s, e)=>s+e['sysTotal']);
    ...
    if (widget.importType == "PURCHASE") {
      ...
      for (var it in reviewedItems.where((e) => e['isSelected'])) {
        Medicine m = it['match'];
        double t = it['taxable']; double tax = it['sysTotal'] - t;
        items.add(BillItem(id: DateTime.now().toString() + m.id, srNo: items.length + 1, medicineID: m.id, name: m.name, packing: m.packing, batch: it['batch'], exp: it['exp'], hsn: it['hsn'], mrp: it['mrp'], qty: it['qty'], freeQty: it['free'], rate: it['rate'], gstRate: it['gstPer'], cgst: isLoc ? tax/2 : 0, sgst: isLoc ? tax/2 : 0, igst: isLoc ? 0 : tax, total: it['sysTotal']));
      }
      await ph.finalizeSale(billNo: partyInfoInFile['billNo'], date: DateFormat('dd/MM/yyyy').parse(partyInfoInFile['date']), party: matchedParty!, items: items, total: (finalGross - partyInfoInFile['extraDisc'] + partyInfoInFile['roundOff']), mode: "CREDIT", sourceTag: "P2P", extraDiscount: partyInfoInFile['extraDisc'], roundOff: partyInfoInFile['roundOff']);
    }
