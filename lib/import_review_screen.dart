...
class ImportReviewScreen extends StatefulWidget {
  ...
  final String exchangeMode; // "C2C" or "C2V" (Initial context)
...
class _ImportReviewScreenState extends State<ImportReviewScreen> {
  ...
  late String currentMode; // User can toggle on screen
  bool isLoading = true;

  @override
  void initState() {
    super.initState();
    currentMode = widget.exchangeMode;
    _processUniversalCsv();
  }

  // ===========================================================================
  // 1. THE BRAIN: DYNAMIC 36-COLUMN PARSER
  // ===========================================================================
  void _processUniversalCsv() {
    ...
    var r1 = data[1]; // Sample First Row
    
    // Mapping: SENDER (Col 2-10) is ALWAYS the other party we care about
    senderInfo = {
      ...
      'date': r1[0].toString(),
      'senderGstInFile': r1[3].toString().trim().toUpperCase(), 
    ...
    for (int i = 1; i < data.length; i++) {
      ...
      reviewedItems.add({
        'name': csvName, 'pack': csvPack, 'hsn': row[20].toString(),
        'mfg': row[21].toString().toUpperCase(), 'salt': row[22].toString().toUpperCase(),
        ...
        'match': match, 'isSelected': match != null,
  ...
  // ===========================================================================
  // 2. THE ACTION: SYNC MASTER & FLIP RATES
  // ===========================================================================
  void _finalizeImport(PharoahManager ph) async {
    ...
    // A. Handshake Logic: Identify P2P vs IMP
    String myGst = ph.activeCompany?.gstin.trim().toUpperCase() ?? "";
    String tag = (senderInfo['senderGstInFile'] == myGst) ? "P2P" : "IMP";

    // B. Get or Create Party (Automatic Profile Mapping)
    Party targetParty;
    try {
      targetParty = ph.parties.firstWhere((p) => p.gst.trim().toUpperCase() == senderInfo['gst'] || p.name == senderInfo['name']);
    } catch (e) {
      targetParty = Party(
        id: DateTime.now().millisecondsSinceEpoch.toString(), name: senderInfo['name'], 
        gst: senderInfo['gst'], dl: senderInfo['dl'], pan: senderInfo['pan'], 
        phone: senderInfo['mobile'], email: senderInfo['email'], address: senderInfo['address'],
        city: senderInfo['city'], state: senderInfo['state'],
    ...
    if (widget.importType == "PURCHASE") {
      ...
      for (var it in reviewedItems.where((e) => e['isSelected'])) {
        ...
        // 💡 THE RATE FLIP MAGIC
        // If C2V (External): CSV's Sale Rate becomes our Purchase Rate.
        // If C2C (Internal): CSV's Purchase Rate remains our Purchase Rate.
        double effectivePurRate = (currentMode == "C2V") ? it['saleRateInFile'] : it['purRateInFile'];

        // Sync Pharma Master (Flags, Salt, MFG)
        ...
        if(mIdx != -1) {
          ...
          ph.medicines[mIdx].purRate = effectivePurRate; ph.medicines[mIdx].mrp = it['mrp'];
        }

        // Register Batch Individually (Safe MRP/Price Conflict)
        BatchSyncEngine.registerBatchActivity(ph: ph, productKey: m.identityKey, batchNo: it['batch'], exp: it['exp'], packing: m.packing, mrp: it['mrp'], rate: effectivePurRate);
        ...
        finalItems.add(PurchaseItem(
          ...
          hsn: it['hsn'], mrp: it['mrp'], qty: it['qty'], freeQty: it['free'], purchaseRate: effectivePurRate,
          gstRate: it['gstPer'], total: (effectivePurRate * it['qty']) * (1 + it['gstPer'] / 100)
        ));
      }
      ...
      ph.finalizePurchase(
        ...
        party: targetParty, items: finalItems, total: finalItems.fold(0, (s, i) => s + i.total), mode: "CREDIT", sourceTag: tag
      );
    } 
    else {
      // SALE IMPORT LOGIC (INTERNAL RECOVERY)
      List<BillItem> finalItems = [];
      for (var it in reviewedItems.where((e) => e['isSelected'])) {
        Medicine m = it['match'];
        BatchSyncEngine.registerBatchActivity(ph: ph, productKey: m.identityKey, batchNo: it['batch'], exp: it['exp'], packing: m.packing, mrp: it['mrp'], rate: it['saleRateInFile']);
        
        finalItems.add(BillItem(
          id: DateTime.now().toString() + m.id, srNo: finalItems.length + 1,
          medicineID: m.id, name: m.name, packing: m.packing, batch: it['batch'], exp: it['exp'],
          hsn: it['hsn'], mrp: it['mrp'], qty: it['qty'], freeQty: it['free'], rate: it['saleRateInFile'],
          gstRate: it['gstPer'], total: (it['saleRateInFile'] * it['qty']) * (1 + it['gstPer'] / 100)
        ));
      }
      ...
      await ph.finalizeSale(
        ...
        total: (finalItems.fold(0.0, (s, i) => s + i.total) - senderInfo['extraDisc'] + senderInfo['roundOff']), 
        mode: "CREDIT", sourceTag: tag
      );
    }

    await ph.save(); 
    Navigator.pop(context);
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("✅ Sync Successful: Dashboard & Master Updated!"), backgroundColor: Colors.green));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text("Final Review & Mapping"), backgroundColor: Colors.indigo.shade900, foregroundColor: Colors.white),
      body: SingleChildScrollView(
        child: Column(children: [
          _buildModeSelector(),
          _buildSenderCard(),
          const Padding(padding: EdgeInsets.all(15), child: Align(alignment: Alignment.centerLeft, child: Text("REVIEW PRODUCT MATCHES", style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.blueGrey)))),
  ...
  Widget _buildModeSelector() => Container(
    padding: const EdgeInsets.all(15), color: Colors.indigo.withOpacity(0.05),
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      const Text("Import Relationship:", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
      DropdownButton<String>(
        value: currentMode,
        items: const [
          DropdownMenuItem(value: "C2C", child: Text("Store Transfer (C2C)")),
          DropdownMenuItem(value: "C2V", child: Text("Vendor to Supply (C2V)")),
        ],
        onChanged: (v) => setState(() => currentMode = v!),
      )
    ]),
  );
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Row(children: [
            ...
            Text(isMatch ? "System Match Found" : "New - Action Required", style: TextStyle(fontSize: 10, color: isMatch ? Colors.green : Colors.red, fontWeight: FontWeight.bold)),
        ]),
        trailing: it['match'] == null ? IconButton(icon: const Icon(Icons.add_box_rounded, color: Colors.red), onPressed: () async {
          // Pre-filled Product Creation
  ...
  Widget _buildBottomBar(PharoahManager ph) => Container(
    ...
    child: ElevatedButton.icon(
      ...
      label: const Text("FINALIZE IMPORT & SYNC MASTER", style: TextStyle(fontWeight: FontWeight.bold)),
    ),
  );
}
