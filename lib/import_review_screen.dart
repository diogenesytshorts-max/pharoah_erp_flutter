// FILE: lib/import_review_screen.dart (FIXED VERSION)
...
class ImportReviewScreen extends StatefulWidget {
  ...
  const ImportReviewScreen({super.key, required this.csvData, required this.importType});
  @override State<ImportReviewScreen> createState() => _ImportReviewScreenState();
}
...
class _ImportReviewScreenState extends State<ImportReviewScreen> {
  ...
  void _processUniversalCsv() {
    ...
    bool isPur = widget.importType == "PURCHASE";
    int offset = isPur ? 2 : 11; 

    senderInfo = {
      'name': r1[offset].toString().trim().toUpperCase(),
      'gst': r1[offset+1].toString().trim().toUpperCase(),
      'dl': r1[offset+2].toString().trim().toUpperCase(),
      'pan': r1[offset+3].toString().trim().toUpperCase(),
      'mobile': r1[offset+4].toString().trim(),
      'email': r1[offset+5].toString().trim().toLowerCase(),
      'address': r1[offset+6].toString().trim().toUpperCase(),
      'city': r1[offset+7].toString().trim().toUpperCase(),
      'state': r1[offset+8].toString().trim(),
    ...
    for (int i = 1; i < data.length; i++) {
      ...
      Medicine? match;
      try { match = ph.medicines.firstWhere((m) => m.name.trim().toUpperCase() == csvName && m.packing.trim().toUpperCase() == csvPack); } 
      catch (e) { try { match = ph.medicines.firstWhere((m) => m.name.trim().toUpperCase() == csvName); } catch (e) { match = null; } }

      reviewedItems.add({
        'name': csvName, 'pack': csvPack, 'hsn': row[20].toString(), 'mfg': row[21].toString().toUpperCase(),
        'salt': row[22].toString().toUpperCase(), 'form': row[23].toString().toUpperCase(),
        'isNaco': row[24].toString().toUpperCase() == "YES", 'isH1': row[25].toString().toUpperCase() == "YES",
        'batch': row[26].toString().trim(), 'exp': row[27].toString(),
        'qty': double.tryParse(row[28].toString()) ?? 0.0, 'free': double.tryParse(row[29].toString()) ?? 0.0,
        'mrp': double.tryParse(row[30].toString()) ?? 0.0, 'purRate': double.tryParse(row[31].toString()) ?? 0.0,
        'saleRate': double.tryParse(row[32].toString()) ?? 0.0, 'gstPer': double.tryParse(row[33].toString()) ?? 12.0,
  ...
  void _finalizeImport(PharoahManager ph) async {
    ...
    Party targetParty;
    try {
      targetParty = ph.parties.firstWhere((p) => p.gst.trim().toUpperCase() == senderInfo['gst'] || (p.name.toUpperCase() == senderInfo['name'].toString().toUpperCase()));
    } catch (e) {
      targetParty = Party(
        id: DateTime.now().millisecondsSinceEpoch.toString(), name: senderInfo['name'], gst: senderInfo['gst'], 
        dl: senderInfo['dl'], pan: senderInfo['pan'], phone: senderInfo['mobile'], email: senderInfo['email'], 
        address: senderInfo['address'], city: senderInfo['city'], state: senderInfo['state'],
    ...
    if (widget.importType == "PURCHASE") {
      ...
      for (var it in reviewedItems.where((e) => e['isSelected'])) {
        Medicine m = it['match'];
        String mfgId = ph.getOrCreateCompany(it['mfg']); String saltId = ph.getOrCreateSalt(it['salt']);
        int mIdx = ph.medicines.indexWhere((med) => med.id == m.id);
        if(mIdx != -1) { 
           ph.medicines[mIdx].companyId = mfgId; ph.medicines[mIdx].saltId = saltId;
           ph.medicines[mIdx].isNarcotic = it['isNaco']; ph.medicines[mIdx].isScheduleH1 = it['isH1'];
           ph.medicines[mIdx].purRate = it['purRate']; ph.medicines[mIdx].mrp = it['mrp']; ph.medicines[mIdx].hsnCode = it['hsn'];
        }
        ...
        finalItems.add(PurchaseItem(id: DateTime.now().toString()+m.id, srNo: finalItems.length+1, medicineID: m.id, name: m.name, packing: m.packing, batch: it['batch'], exp: it['exp'], hsn: it['hsn'], mrp: it['mrp'], qty: it['qty'], freeQty: it['free'], purchaseRate: it['purRate'], gstRate: it['gstPer'], total: (it['purRate'] * it['qty']) * (1 + it['gstPer'] / 100)));
      }
      ph.finalizePurchase(internalNo: "P2P-${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}", billNo: "${senderInfo['billNo']} [P2P-IMP]", date: DateFormat('dd/MM/yyyy').parse(senderInfo['date']), entryDate: DateTime.now(), party: targetParty, items: finalItems, total: finalItems.fold(0.0, (s, i) => s + i.total), mode: "CREDIT", isImported: true);
    } else {
      ...
      for (var it in reviewedItems.where((e) => e['isSelected'])) {
        ...
        finalItems.add(BillItem(id: DateTime.now().toString()+m.id, srNo: finalItems.length+1, medicineID: m.id, name: m.name, packing: m.packing, batch: it['batch'], exp: it['exp'], hsn: it['hsn'], mrp: it['mrp'], qty: it['qty'], freeQty: it['free'], rate: it['saleRate'], gstRate: it['gstPer'], total: (it['saleRate'] * it['qty']) * (1 + it['gstPer'] / 100)));
      }
      
      double grossTotal = finalItems.fold(0.0, (s, i) => s + i.total);
      double extraDisc = senderInfo['extraDiscount'] as double;
      double roundOffVal = senderInfo['roundOff'] as double;

      await ph.finalizeSale(
        billNo: "${senderInfo['billNo']} [P2P-IMP]", 
        date: DateFormat('dd/MM/yyyy').parse(senderInfo['date']), 
        party: targetParty, 
        items: finalItems, 
        total: (grossTotal - extraDisc + roundOffVal), 
        mode: "CREDIT", 
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("✅ P2P Import Complete! All Details Mapped."), backgroundColor: Colors.green));
  }
  ...
  Widget _buildSenderHeader() => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(senderInfo['name'], style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 16, color: Colors.indigo)), Text(senderInfo['billNo'], style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blueGrey))]),
      const Divider(),
  ...
  Widget _buildItemRow(Map<String, dynamic> it) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: BorderSide(color: it['isSelected'] ? Colors.green.shade200 : Colors.grey.shade300)
      ),
      child: ListTile(
