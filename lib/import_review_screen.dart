...
class _ImportReviewScreenState extends State<ImportReviewScreen> {
  ...
  // ===========================================================================
  // 1. THE BRAIN: DYNAMIC 36-COLUMN PARSER (Corrects Party vs Company)
  // ===========================================================================
  void _processUniversalCsv() {
    ...
    // logic: P2P means Sender of File (Col 2-10) is our target Party.
    senderInfo = {
    ...
    for (int i = 1; i < data.length; i++) {
      ...
      reviewedItems.add({
        'name': csvName, 
        'pack': csvPack, // Key changed to match ProductMaster auto-fill
        'hsn': row[20].toString(),
  ...
  // ===========================================================================
  // 2. THE ACTION: SYNC MASTER & SAVE WITH SOURCE TAGS
  // ===========================================================================
  void _finalizeImport(PharoahManager ph) async {
    if (reviewedItems.any((it) => it['isSelected'] && it['match'] == null)) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Link or Create all selected products first!")));
      return;
    }
    ...
    try {
      targetParty = ph.parties.firstWhere((p) => 
        p.gst.trim().toUpperCase() == senderInfo['gst'] || p.name == senderInfo['name']
      );
    } catch (e) {
      targetParty = Party(
    ...
    if (widget.importType == "PURCHASE") {
      ...
      for (var it in reviewedItems.where((e) => e['isSelected'])) {
        Medicine m = it['match'];
        double effRate = (currentMode == "C2V") ? it['saleRateInFile'] : it['purRateInFile'];
        ...
        if(mIdx != -1) {
          ...
          ph.medicines[mIdx].purRate = effRate;
          ph.medicines[mIdx].mrp = it['mrp'];
        }

        BatchSyncEngine.registerBatchActivity(ph: ph, productKey: m.identityKey, batchNo: it['batch'], exp: it['exp'], packing: m.packing, mrp: it['mrp'], rate: effRate);
        ...
        finalItems.add(PurchaseItem(
          ...
          hsn: it['hsn'], mrp: it['mrp'], qty: it['qty'], freeQty: it['free'], purchaseRate: effRate,
          gstRate: it['gstPer'], total: (effRate * it['qty']) * (1 + it['gstPer'] / 100)
        ));
      }
      ...
      await ph.finalizeSale(
        ...
        total: (gross - senderInfo['extraDisc'] + senderInfo['roundOff']), 
        mode: "CREDIT", sourceTag: tag,
        extraDiscount: senderInfo['extraDisc'], roundOff: senderInfo['roundOff']
      );
    }

    await ph.save(); 
    Navigator.pop(context);
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("✅ Import Sync Complete!"), backgroundColor: Colors.green));
  }
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text("Batch: ${it['batch']} | MRP: ${it['mrp']} | Rate: ${it['purRateInFile']}", style: const TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: Colors.blueGrey)),
          Row(children: [
  ...
  Widget _buildBottomBar(PharoahManager ph) => Container(
    ...
    child: ElevatedButton.icon(
      ...
      label: const Text("FINALIZE IMPORT & SYNC DATA", style: TextStyle(fontWeight: FontWeight.bold)),
    ),
  );
}
