...
class SaleChallanPdf {
  // Items count synced with Signature View
  ...
  static Future<Uint8List> generateBytes(SaleChallan challan, Party party, CompanyProfile shop) async {
    ...
    if (challan.isSigned && challan.sigHistory.isNotEmpty) {
      final lastSig = challan.sigHistory.last;
      ...
      pdf.addPage(pw.Page(
          ...
          build: (context) => pw.Stack(children: [
            pw.Column(children: [
              ...
              // Footer
              if (isLastPage) _buildFooter(shop.name, challan.totalAmount, challan.remarks, challan.isSigned ? challan.sigHistory.last.verificationCode : null)
              else pw.Align(alignment: pw.Alignment.centerRight, child: pw.Text("Continued...", style: const pw.TextStyle(fontSize: 10))),
            ]),
            // Signature Position Sync
            if (isLastPage && sigImage != null)
               pw.Positioned(bottom: 20, right: 50, child: pw.Image(sigImage, width: 150)),
            // Watermark
            if (challan.isSigned)
               pw.Center(child: pw.Opacity(opacity: 0.1, child: pw.Transform.rotate(angle: -0.5, child: pw.Text(challan.sigHistory.last.verificationCode, style: pw.TextStyle(fontSize: 70, fontWeight: pw.FontWeight.bold, color: PdfColors.red900))))),
          ]),
      ));
    }
    return pdf.save();
  ...
  static pw.Widget _tCol(String t, double w, {bool isLast = false, bool isLeft = false}) => pw.Container(width: w, height: 20, alignment: isLeft ? pw.Alignment.centerLeft : pw.Alignment.center, padding: pw.EdgeInsets.only(left: 5), decoration: pw.BoxDecoration(border: pw.Border(right: pw.BorderSide(width: isLast ? 0 : 0.5), bottom: const pw.BorderSide(width: 0.5))), child: pw.Text(t, style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)));
  ...
  static pw.Widget _buildFooter(String n, double t, String r, String? code) => pw.Container(height: 100, decoration: const pw.BoxDecoration(border: pw.Border(top: pw.BorderSide(width: 0.5))), child: pw.Row(children: [
    pw.Container(width: 480, padding: const pw.EdgeInsets.all(8), decoration: const pw.BoxDecoration(border: pw.Border(right: pw.BorderSide(width: 0.5))), child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
      if (code != null) pw.Text("DIGITAL SEAL: $code", style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold, color: PdfColors.red900)),
      pw.Text("REMARKS: ${r.isEmpty ? 'N/A' : r}", style: const pw.TextStyle(fontSize: 8)),
      pw.Spacer(), pw.Text("Note: This is a digitally signed delivery document.", style: const pw.TextStyle(fontSize: 7)),
    ])),
    pw.Container(width: 320, padding: const pw.EdgeInsets.all(8), child: pw.Column(children: [
      ...
      pw.Spacer(), pw.Align(alignment: pw.Alignment.bottomRight, child: pw.Text("For $n", style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold))),
    ])),
  ]));
}
