...
class SaleChallanPdf {
          ...
          build: (context) => pw.Stack(children: [
            pw.Column(children: [
              ...
              pw.Container(color: PdfColors.grey200, child: pw.Row(children: [
                _tCol("S.N", 25), _tCol("Qty+Free", 50), _tCol("Pack", 40), _tCol("Product Description", 210, isLeft: true), 
                _tCol("Batch", 75), _tCol("Exp", 45), _tCol("HSN", 45), _tCol("MRP", 55), 
                _tCol("Rate", 55), _tCol("GST%", 35), _tCol("Net Total", 145, isLast: true), 
              ])),
              ...
              pw.Expanded(child: pw.Column(children: pageItems.asMap().entries.map((entry) {
                ...
                return pw.Container(decoration: const pw.BoxDecoration(border: pw.Border(bottom: pw.BorderSide(width: 0.1, color: PdfColors.grey400))), child: pw.Row(children: [
                  _cell("$sn", 25), _cell("${i.qty.toInt()} + ${i.freeQty.toInt()}", 50), _cell(i.packing, 40), _cell(i.name, 210, isLeft: true), _cell(i.batch, 75), _cell(i.exp, 45), _cell(i.hsn, 45), _cell(i.mrp.toStringAsFixed(2), 55), _cell(i.rate.toStringAsFixed(2), 55), _cell("${i.gstRate.toInt()}%", 35), _cell(i.total.toStringAsFixed(2), 145),
                ]));
              }).toList())),

              if (isLastPage) _buildFooter(shop.name, challan.totalAmount, challan.remarks, lastSig)
              else pw.Align(alignment: pw.Alignment.centerRight, child: pw.Text("Continued on next page...", style: const pw.TextStyle(fontSize: 10))),
            ]),
            
            // SIGNATURE (Bottom Left - Receiver Box)
            if (isLastPage && sigImage != null)
               pw.Positioned(bottom: 45, left: 30, child: pw.Image(sigImage, width: 140)),

            // Watermark (Locked Code)
            if (challan.isSigned)
  ...
  static pw.Widget _buildFooter(String n, double t, String r, ChallanSignature? sig) => pw.Container(height: 110, decoration: const pw.BoxDecoration(border: pw.Border(top: pw.BorderSide(width: 0.5))), child: pw.Row(children: [
    pw.Container(width: 480, padding: const pw.EdgeInsets.all(5), decoration: const pw.BoxDecoration(border: pw.Border(right: pw.BorderSide(width: 0.5))), child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
      ...
      pw.Spacer(), 
      pw.Text("SECURE DOCUMENT NOTICE: This document is digitally locked. Any post-signing modification in quantity or amount will invalidate Seal: ${sig?.verificationCode ?? 'N/A'}.", style: pw.TextStyle(fontSize: 5.5, color: PdfColors.grey700, fontStyle: pw.FontStyle.italic)),
    ])),
