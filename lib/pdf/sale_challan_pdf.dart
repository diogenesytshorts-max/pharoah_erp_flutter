...
class SaleChallanPdf {
  ...
  // 2. GENERATE BYTES (For Save/Share)
                ...
                pw.Column(children: [
                  pw.Row(children: [
                    ...
                    _hBox(335, false, pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                      pw.Text("CONSIGNEE:", style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: PdfColors.grey)),
                  ...
                  pw.Container(color: PdfColors.grey200, child: pw.Row(children: [
                      ...
                      _tCol("Batch", 75), _tCol("Exp", 45), _tCol("HSN", 50), _tCol("MRP", 55), _tCol("Rate", 55), _tCol("GST%", 30), _tCol("Net Amount", 155, isLast: true), 
                  ])),
                  pw.Expanded(child: pw.Column(children: pageItems.map((i) => pw.Container(decoration: const pw.BoxDecoration(border: pw.Border(bottom: pw.BorderSide(width: 0.1, color: PdfColors.grey400))), child: pw.Row(children: [
                          _cell("${challan.items.indexOf(i) + 1}", 25), _cell("${i.qty.toInt()} + ${i.freeQty.toInt()}", 55), _cell(i.packing, 45), _cell(i.name, 210, isLeft: true), _cell(i.batch, 75), _cell(i.exp, 45), _cell(i.hsn, 50), _cell(i.mrp.toStringAsFixed(2), 55), _cell(i.rate.toStringAsFixed(2), 55), _cell("${i.gstRate.toInt()}%", 30), _cell(i.total.toStringAsFixed(2), 155),
                  ]))).toList())),
                  if (isLastPage) _buildFinalFooter(shop.name, challan.totalAmount, challan.remarks, lastSig)
                  else _buildIntermediateFooter(pageNum + 1, lastSig),
                ]),
                if (lastSig != null) pw.Center(child: pw.Opacity(opacity: 0.1, child: pw.Transform.rotate(angle: -0.5, child: pw.Text(lastSig.verificationCode, style: pw.TextStyle(fontSize: 70, fontWeight: pw.FontWeight.bold, color: PdfColors.red900))))),
                if (isLastPage && sigImage != null && lastSig != null) pw.Positioned(left: (lastSig.signX * masterWidth) - 60, top: (lastSig.signY * pageHeightLimit) - 30, child: pw.Image(sigImage, width: 120)),
              ])),
      ));
    }
    return pdf.save();
  ...
  static pw.Widget _buildIntermediateFooter(int p, ChallanSignature? s) => pw.Container(height: 60, padding: const pw.EdgeInsets.symmetric(horizontal: 10), decoration: const pw.BoxDecoration(border: pw.Border(top: pw.BorderSide(width: 0.5))), child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [if (s != null) _buildSecurityMessage(s.verificationCode), pw.Text("Continued to Page ${p + 1}...", style: pw.TextStyle(fontSize: 9, fontStyle: pw.FontStyle.italic))]));
  static pw.Widget _buildFinalFooter(String n, double t, String r, ChallanSignature? s) => pw.Container(height: 100, decoration: const pw.BoxDecoration(border: pw.Border(top: pw.BorderSide(width: 0.5))), child: pw.Row(children: [
      pw.Container(width: 480, padding: const pw.EdgeInsets.all(5), decoration: const pw.BoxDecoration(border: pw.Border(right: pw.BorderSide(width: 0.5))), child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [if (s != null) _buildSecurityMessage(s.verificationCode), pw.SizedBox(height: 5), pw.Text("REMARKS: ${r.isEmpty ? 'Condition verified.' : r}", style: const pw.TextStyle(fontSize: 7.5)), pw.Spacer(), pw.Text("Note: This is a digitally verified delivery challan.", style: const pw.TextStyle(fontSize: 6.5, color: PdfColors.grey700))])),
      pw.Container(width: 320, padding: const pw.EdgeInsets.all(5), child: pw.Column(children: [pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text("GRAND TOTAL VALUE", style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)), pw.Text("Rs. ${t.toStringAsFixed(2)}", style: pw.TextStyle(fontSize: 13, fontWeight: pw.FontWeight.bold))]), pw.Spacer(), pw.Align(alignment: pw.Alignment.bottomRight, child: pw.Column(children: [pw.Text("For $n", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)), pw.SizedBox(height: 10), pw.Text("Authorised Signatory", style: const pw.TextStyle(fontSize: 7))]))]))]));
  static pw.Widget _buildSecurityMessage(String c) => pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [pw.Text("DIGITAL SEAL: $c", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: PdfColors.red900)), pw.SizedBox(width: 320, child: pw.Text("SECURITY NOTICE: This document is locked with code $c. Unauthorized modification will invalidate this digital seal.", style: const pw.TextStyle(fontSize: 5.5, color: PdfColors.grey700)))]);
}
