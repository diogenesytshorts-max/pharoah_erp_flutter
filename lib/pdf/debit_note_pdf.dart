...
class DebitNotePdf {
  ...
  // --- 🏛️ ARCHITECT LANDSCAPE (Purchase Return Mapping) ---
        ...
        build: (context) => pw.Container(
          ...
          child: pw.Column(children: [
            ...
            pw.Container(color: PdfColors.grey200, child: pw.Row(children: [
              _tCol("S.N", 25), _tCol("Qty+Free", 60), _tCol("Pack", 40), _tCol("Product Name", 215, isLeft: true), _tCol("Batch", 70), _tCol("Exp", 45), _tCol("MRP", 60), _tCol("Rate", 60), _tCol("GST%", 50), _tCol("Total", 125, isLast: true), 
            ])),
            pw.Expanded(child: pw.Column(children: pageContent.map((row) {
              ...
              return pw.Container(color: isShaded ? PdfColors.grey50 : PdfColors.white, decoration: const pw.BoxDecoration(border: pw.Border(bottom: pw.BorderSide(width: 0.1))), child: pw.Row(children: [
                  ...
                  _cell(i.batch, 75), _cell(i.exp, 45), _cell(i.mrp.toStringAsFixed(2), 60), _cell(i.purchaseRate.toStringAsFixed(2), 60), _cell("${i.gstRate.toInt()}%", 50), _cell(i.total.toStringAsFixed(2), 125),
              ]));
            }).toList())),
  ...
  static Future<void> _generateStandard(PurchaseReturn ret, Party supplier, CompanyProfile shop, AppConfig config) async {
    ...
    pdf.addPage(pw.MultiPage(
      ...
      build: (context) => [
        pw.Text("DEBIT NOTE - ${shop.name}", style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold, color: PdfColors.brown900)),
        pw.SizedBox(height: 10),
        pw.Text("To: ${supplier.name} | GST: ${supplier.gst}"),
  ...
  static pw.Widget _buildArchitectFooter(String shopName, PurchaseReturn ret) => pw.Container(height: 100, decoration: const pw.BoxDecoration(border: pw.Border(top: pw.BorderSide(width: 0.5))), child: pw.Row(children: [
    pw.Container(width: 450, padding: const pw.EdgeInsets.all(5), decoration: const pw.BoxDecoration(border: pw.Border(right: pw.BorderSide(width: 0.5))), child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
        ...
        pw.Spacer(),
        pw.Text("Generated for return adjustements.", style: const pw.TextStyle(fontSize: 6))
    ])),
    pw.Expanded(child: pw.Column(children: [
      pw.Padding(padding: const pw.EdgeInsets.all(8), child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text("NET DEBIT TOTAL", style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)), pw.Text("Rs. ${ret.totalAmount.toStringAsFixed(2)}", style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold, color: PdfColors.brown900))])),
      pw.Spacer(),
      pw.Align(alignment: pw.Alignment.bottomRight, child: pw.Padding(padding: const pw.EdgeInsets.all(8), child: pw.Text("For $shopName", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold))))
    ])),
  ]));
}
