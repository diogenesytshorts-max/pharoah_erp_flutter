...
class CreditNotePdf {
  ...
  // --- 🏛️ ARCHITECT LANDSCAPE (Mirror of Architect Sale Bill) ---
  static Future<void> _generateArchitect(SaleReturn ret, Party party, CompanyProfile shop, AppConfig config) async {
          ...
          child: pw.Column(children: [
            pw.Row(children: [
              ...
              _hBox(345, false, pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                ...
                pw.Text("GST: ${party.gst} | City: ${party.city}", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)),
              ])),
            ]),
            ...
            pw.Expanded(child: pw.Column(children: pageContent.map((row) {
              ...
              int originalIndex = ret.items.indexOf(i);
              bool isShaded = config.useZebraShading && (originalIndex % 2 != 0);
              return pw.Container(color: isShaded ? PdfColors.grey50 : PdfColors.white, child: pw.Row(children: [
                  _cell("${originalIndex + 1}", 25), _cell("${i.qty.toInt()}+${i.freeQty.toInt()}", 50), _cell(i.packing, 40),
    ...
    await Printing.layoutPdf(onLayout: (f) async => pdf.save(), name: 'CreditNote_${ret.billNo}', format: PdfPageFormat.a4.landscape);
  }

  // --- Standard Logic (Single MultiPage Portrait) ---
  static Future<void> _generateStandard(SaleReturn ret, Party party, CompanyProfile shop, AppConfig config) async {
    final pdf = pw.Document();
    pdf.addPage(pw.MultiPage(
      pageFormat: PdfPageFormat.a4,
      build: (context) => [
        pw.Text("CREDIT NOTE (SALE RETURN) - ${shop.name}", style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold, color: PdfColors.red900)),
        pw.Divider(),
        pw.Text("Return No: ${ret.billNo} | Date: ${DateFormat('dd/MM/yy').format(ret.date)}"),
        pw.Text("Party: ${party.name} | GST: ${party.gst}"),
        pw.SizedBox(height: 20),
        pw.TableHelper.fromTextArray(
          headers: ['Product', 'Batch', 'Qty', 'Rate', 'Total'],
          data: ret.items.map((i) => [i.name, i.batch, i.qty.toString(), i.rate.toStringAsFixed(2), i.total.toStringAsFixed(2)]).toList(),
        ),
        pw.Divider(),
        pw.Align(alignment: pw.Alignment.centerRight, child: pw.Text("NET CREDIT: Rs. ${ret.totalAmount.toStringAsFixed(2)}", style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold))),
      ],
    ));
    await Printing.layoutPdf(onLayout: (f) async => pdf.save());
  }

  // Helpers with FIXED EdgeInsets and padding
  ...
  static pw.Widget _buildArchitectFooter(String shopName, SaleReturn ret, bool isLocal) {
    ...
    return pw.Container(height: 100, decoration: const pw.BoxDecoration(border: pw.Border(top: pw.BorderSide(width: 0.5))), child: pw.Row(children: [
      pw.Container(width: 320, padding: const pw.EdgeInsets.all(5), decoration: const pw.BoxDecoration(border: pw.Border(right: pw.BorderSide(width: 0.5))), child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
        pw.Text("Amt: RUPEES ${PdfMasterService.numberToWords(ret.totalAmount.round())} ONLY", style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: PdfColors.blue800)),
        pw.Spacer(),
        pw.Text("Internal record for account adjustment only.", style: const pw.TextStyle(fontSize: 6)),
      ])),
      pw.Container(width: 250, padding: const pw.EdgeInsets.all(5), decoration: const pw.BoxDecoration(border: pw.Border(right: pw.BorderSide(width: 0.5))), child: pw.Column(children: [
        ...
        pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
          ...
          pw.Text("Rs. ${ret.totalAmount.toStringAsFixed(2)}", style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold, color: PdfColors.red900)),
        ]),
      ])),
