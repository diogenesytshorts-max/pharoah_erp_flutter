...
class _DataExchangeViewState extends State<DataExchangeView> {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _buildMainActionCard(title: "PHAROAH CSV", subtitle: "Import/Export in native Pharoah format for perfect sync.", icon: Icons.account_tree_rounded, color: Colors.blue.shade800, onTap: () => _showOptions(context, ph, "PHAROAH")),
          const SizedBox(height: 20),
          _buildMainActionCard(title: "OTHER CSV", subtitle: "Import from Distributors or generic Excel sheets.", icon: Icons.grid_on_rounded, color: Colors.teal.shade700, onTap: () => _showOptions(context, ph, "OTHER")),
        ]),
      ),
    );
  }
  ...
  void _showOptions(BuildContext context, PharoahManager ph, String mode) {
    showModalBottomSheet(context: context, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(25))), builder: (c) => Container(
      padding: const EdgeInsets.all(25), child: Column(mainAxisSize: MainAxisSize.min, children: [
  ...
  Widget _buildMainActionCard({required String title, required String subtitle, required IconData icon, required Color color, required VoidCallback onTap}) {
    return InkWell(onTap: onTap, borderRadius: BorderRadius.circular(20), child: Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: color.withOpacity(0.2)), boxShadow: [BoxShadow(color: color.withOpacity(0.05), blurRadius: 10, offset: const Offset(0, 5))]), child: Row(children: [Container(padding: const EdgeInsets.all(15), decoration: BoxDecoration(color: color.withOpacity(0.1), shape: BoxShape.circle), child: Icon(icon, color: color, size: 30)), const SizedBox(width: 20), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: color)), const SizedBox(height: 5), Text(subtitle, style: const TextStyle(fontSize: 12, color: Colors.grey))])), const Icon(Icons.arrow_forward_ios_rounded, size: 16, color: Colors.grey)])));
  }
