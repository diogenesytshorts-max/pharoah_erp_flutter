...
class PdfService {
  static Future<void> generateInvoice(Sale sale, Party party, {bool isPurchase = false}) async {
    ...
    for (var item in sale.items) {
      ...
      grandTotalGST += (item.cgst + item.sgst);

      if (!gstBuckets.containsKey(rate)) {
        gstBuckets[rate] = {'taxable': 0, 'cgst': 0, 'sgst': 0};
      }
            ...
            child: pw.Column(children: [
              pw.Row(children: [
                ...
                pw.Container(width: 180, height: 100, decoration: pw.BoxDecoration(border: pw.Border.all(width: 0.5)), child: pw.Column(children: [
                  pw.Container(width: double.infinity, color: isPurchase ? PdfColors.orange100 : PdfColors.blue100, padding: const pw.EdgeInsets.all(4), child: pw.Center(child: pw.Text(isPurchase ? "PURCHASE BILL" : "GST INVOICE", style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)))),
                  ...
                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                    pw.Text("Bill No: ${sale.billNo}", style: const pw.TextStyle(fontSize: 9)),
                    pw.Text("Date: ${DateFormat('dd/MM/yyyy').format(sale.date)}", style: const pw.TextStyle(fontSize: 9)),
                    pw.Text("Page ${pageIndex + 1} of $totalPages", style: pw.TextStyle(fontSize: 7, color: PdfColors.blue800)),
                  ])),
                ])),
                pw.Container(width: 322, height: 100, padding: const pw.EdgeInsets.all(8), decoration: pw.BoxDecoration(border: pw.Border.all(width: 0.5)), child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                  pw.Text(isPurchase ? "FROM / DISTRIBUTOR:" : "BILL TO / PARTY DETAILS:", style: pw.TextStyle(fontSize: 7, color: PdfColors.grey700)),
                  ...
                  pw.Text("Phone: ${party.phone} | D.L.No: ${party.dl}", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)),
                ])),
              ]),
              pw.Container(color: PdfColors.grey100, child: pw.Row(children: [
                _c("S.N", 30), _c("Qty", 45), _c("Pack", 45), _c("Product Name", 180, a: pw.Alignment.centerLeft), _c("Batch", 80), _c("Exp", 50), _c("HSN", 50), _c("MRP", 60), _c(isPurchase ? "Pur.Rate" : "Rate", 60), _c("DIS%", 35), _c("SGST%", 45), _c("CGST%", 45), _c("Net Amt", 77),
              ])),
              ...
              if (pageIndex == totalPages - 1) ...[
                pw.Row(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                  pw.Container(width: 340, child: pw.Column(children: [
                    ...
                    ...gstBuckets.entries.map((entry) => pw.Row(children: [ _sC("GST ${entry.key.toInt()}%", 70), _sC(entry.value['taxable']!.toStringAsFixed(2), 80), _sC(entry.value['sgst']!.toStringAsFixed(2), 60), _sC(entry.value['cgst']!.toStringAsFixed(2), 60), _sC((entry.value['sgst']! + entry.value['cgst']!).toStringAsFixed(2), 70)])),
                  ])),
                  pw.Spacer(),
                  pw.Container(width: 200, height: 100, decoration: pw.BoxDecoration(border: pw.Border.all(width: 1)), child: pw.Column(children: [
                    _fR("GROSS TOTAL", grandTotalGross), _fR("GST PAYABLE", grandTotalGST),
                    pw.Spacer(),
                    pw.Container(width: double.infinity, color: PdfColors.grey100, padding: const pw.EdgeInsets.all(5), child: pw.Column(children: [
    ...
    await Printing.layoutPdf(onLayout: (f) async => pdf.save(), name: "Purchase_${sale.billNo}", format: PdfPageFormat.a4.landscape);
  }

  static pw.Widget _c(String t, double w, {pw.Alignment a = pw.Alignment.center}) => pw.Container(width: w, height: 25, alignment: a, decoration: pw.BoxDecoration(border: pw.Border.all(width: 0.5)), child: pw.Text(t, style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: PdfColors.black)));
