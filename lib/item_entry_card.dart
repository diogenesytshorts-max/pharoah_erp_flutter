...
class _ItemEntryCardState extends State<ItemEntryCard> {
  ...
  void _validateAndAdd(PharoahManager ph) {
    ...
    // BATCH SYNC: CASE SENSITIVE SAFE (Mirroring)
    BatchSyncEngine.registerBatchActivity(
      ph: ph, 
      productKey: widget.med.identityKey, 
      batchNo: batchC.text.trim(), // Exact Case preserved
      exp: expC.text, 
  ...
  void _addItemToBill() {
    ...
    widget.onAdd(BillItem(
      ...
      packing: widget.med.packing,
      batch: batchC.text.trim(), // FIXED: No toUpperCase()
      exp: expC.text,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(child: _buildInput("QTY", qtyC, isNum: true, isReadOnly: widget.existingItem?.sourceChallanNo.isNotEmpty ?? false, onChanged: (v) => setState((){}))),
              const SizedBox(width: 8),
