...
class _BillingViewState extends State<BillingView> {
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        Column(children: [
          ...
          Expanded(child: ListView.builder(itemCount: items.length, itemBuilder: (c, i) => ListTile(title: Text(items[i].name, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text("Qty: ${items[i].qty.toInt()} | Rate: ${items[i].rate}"), trailing: Text("₹${items[i].total.toStringAsFixed(2)}", style: const TextStyle(fontWeight: FontWeight.bold)), onTap: widget.isReadOnly ? null : () => setState(() { selectedMed = ph.medicines.firstWhere((m) => m.id == items[i].medicineID); editingIndex = i; })))),
          Container(padding: const EdgeInsets.all(15), color: Colors.blue[50], child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Items: ${items.length}"), Text("TOTAL: ₹${grandTotal.toStringAsFixed(2)}", style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.green))]))
        ]),
  ...
  void _saveAndClose(PharoahManager ph) async { if (widget.modifySaleId != null) ph.deleteBill(widget.modifySaleId!); ph.finalizeSale(billNo: widget.billNo, date: widget.billDate, party: widget.party, items: items, total: grandTotal, mode: widget.mode); if (widget.modifySaleId == null) await SaleBillNumber.increment(); Navigator.of(context).pop(); }
  void _saveAndPrint(PharoahManager ph) async { final sale = Sale(id: DateTime.now().toString(), billNo: widget.billNo, date: widget.billDate, partyName: widget.party.name, items: items, totalAmount: grandTotal, paymentMode: widget.mode); if (!widget.isReadOnly) { if (widget.modifySaleId != null) ph.deleteBill(widget.modifySaleId!); ph.finalizeSale(billNo: widget.billNo, date: widget.billDate, party: widget.party, items: items, total: grandTotal, mode: widget.mode); if (widget.modifySaleId == null) await SaleBillNumber.increment(); } await PdfService.generateInvoice(sale, widget.party); if (!widget.isReadOnly) Navigator.of(context).pop(); }
}
...
class _ItemEntryFormState extends State<ItemEntryForm> {
  ...
  final rateCDiscC = TextEditingController(text: "0.0"); final normalDiscPC = TextEditingController(text: "0.0"); final normalDiscRC = TextEditingController(text: "0.0");
  String rateType = "A";
  ...
  @override void initState() {
    ...
    if (widget.existingItem != null) {
      ...
      qtyC.text = widget.existingItem!.qty.toString(); normalDiscPC.text = widget.existingItem!.discountPercent.toString(); normalDiscRC.text = widget.existingItem!.discountRupees.toString();
    } else {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Container(padding: const EdgeInsets.all(10), color: Colors.white, child: Column(children: [
      ...
      if (history.isNotEmpty) SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: history.map((b)=>ActionChip(label: Text(b.batch), onPressed: (){ setState(() { batchC.text = b.batch; expC.text = b.exp; mrpC.text = b.mrp.toString(); rateC.text = b.rate.toString(); }); })).toList())),
      Row(children: [
      ...
      SegmentedButton<String>(segments: const [ButtonSegment(value: 'A', label: Text('Rate A')), ButtonSegment(value: 'B', label: Text('Rate B')), ButtonSegment(value: 'C', label: Text('Rate C'))], selected: {rateType}, onSelectionChanged: (val) { setState(() => rateType = val.first); _updateRate(); }),
      Row(children: [
        if (rateType == 'C') Expanded(child: TextField(controller: rateCDiscC, decoration: const InputDecoration(labelText: "RC Disc%"), keyboardType: TextInputType.number, onChanged: (v)=>_updateRate())),
