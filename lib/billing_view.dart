...
class _BillingViewState extends State<BillingView> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        elevation: 2, backgroundColor: widget.isReadOnly ? Colors.purple : Colors.blue.shade700, foregroundColor: Colors.white,
        ...
        Column(children: [
          ...
          if (selectedMed != null)
            ItemEntryForm(
              ...
              onAdd: (newItem) {
                // Yahan par batch hamesha ke liye save ho jayega
                ...
                setState(() {
                  if (editingIndex != null) items[editingIndex!] = newItem; else items.add(newItem);
  ...
  void _printBill(PharoahManager ph) async {
    final sale = Sale(id: DateTime.now().toString(), billNo: widget.billNo, date: widget.billDate, partyName: widget.party.name, partyGstin: widget.party.gst, partyState: widget.party.state, partyAddress: widget.party.address, partyDl: widget.party.dl, partyEmail: widget.party.email, items: items, totalAmount: grandTotal, paymentMode: widget.mode, invoiceType: widget.party.isB2B ? "B2B" : "B2C");
    await PdfService.generateInvoice(sale, widget.party);
    if (!widget.isReadOnly) { // Sirf naye bill ko save karo print ke sath
      _saveAndClose(ph);
    }
  }
}
