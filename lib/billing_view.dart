...
class _BillingViewState extends State<BillingView> {
        ...
        Column(children: [
          ...
          Expanded(child: ListView.builder(itemCount: items.length, itemBuilder: (c, i) => ListTile(
            ...
            trailing: Row(mainAxisSize: MainAxisSize.min, children: [
              Text("₹${items[i].total.toStringAsFixed(2)}", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)),
              if (!widget.isReadOnly) IconButton(icon: const Icon(Icons.copy, size: 18, color: Colors.orange), onPressed: () => setState(() { selectedMed = ph.medicines.firstWhere((m) => m.id == items[i].medicineID); editingIndex = null; })),
            ]),
          ...
          Container(padding: const EdgeInsets.all(12), color: Colors.blue[50], child: Column(children: [
            ...
            const Divider(),
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Items: ${items.length}"), Text("TOTAL: ₹${grandTotal.toStringAsFixed(2)}", style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.green))]),
          ]))
        ]),
  ...
  void _saveAndClose(PharoahManager ph) async { if (widget.modifySaleId != null) ph.deleteBill(widget.modifySaleId!); ph.finalizeSale(bNo: widget.billNo, dt: widget.billDate, p: widget.party, its: items, tot: grandTotal, md: widget.mode); if (widget.modifySaleId == null) await SaleBillNumber.increment(); if(mounted) Navigator.pop(context); }
  void _saveAndPrint(PharoahManager ph) async { final sale = Sale(id: DateTime.now().toString(), billNo: widget.billNo, date: widget.billDate, partyName: widget.party.name, items: items, totalAmount: grandTotal, paymentMode: widget.mode); if (!widget.isReadOnly) { if (widget.modifySaleId != null) ph.deleteBill(widget.modifySaleId!); ph.finalizeSale(bNo: widget.billNo, dt: widget.billDate, p: widget.party, its: items, tot: grandTotal, md: widget.mode); if (widget.modifySaleId == null) await SaleBillNumber.increment(); } await PdfService.generateInvoice(sale, widget.party); if (!widget.isReadOnly && mounted) Navigator.pop(context); }
}
...
class _ItemEntryFormState extends State<ItemEntryForm> {
  ...
  @override void initState() {
    ...
    eC.addListener(() {
      ...
      if (t.length >= 2) {
        int month = int.tryParse(t.substring(0, 2)) ?? 0;
  ...
  @override Widget build(BuildContext context) {
    final ph = Provider.of<PharoahManager>(context);
    final h = ph.batchHistory[widget.med.id] ?? [];
    return Container(padding: const EdgeInsets.all(10), color: Colors.white, child: Column(children: [
      ...
      if (h.isNotEmpty) SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: h.map((b)=>ActionChip(label: Text(b.batch), onPressed: (){ setState(() { bC.text = b.batch; eC.text = b.exp; mC.text = b.mrp.toString(); rC.text = b.rate.toString(); }); })).toList())),
      Row(children: [
        Expanded(child: TextField(controller: bC, decoration: const InputDecoration(labelText: "Batch"))), const SizedBox(width: 5),
        Expanded(child: TextField(controller: eC, decoration: const InputDecoration(hintText: "MM/YY", labelText: "Exp"), keyboardType: TextInputType.number)), const SizedBox(width: 5),
        Expanded(child: TextField(controller: gC, decoration: const InputDecoration(labelText: "GST%"), keyboardType: TextInputType.number, onChanged: (v)=>_upd())),
      ]),
      ...
      Row(children: [
        if (rT == 'C') Expanded(child: TextField(controller: rCD, decoration: const InputDecoration(labelText: "RC Disc%"), keyboardType: TextInputType.number, onChanged: (v)=>_upd())),
        Expanded(child: TextField(controller: mC, decoration: const InputDecoration(labelText: "MRP"), keyboardType: TextInputType.number, onChanged: (v) => _upd())),
      ...
      Row(children: [
        Expanded(child: TextField(controller: nDP, decoration: const InputDecoration(labelText: "Disc %"), keyboardType: TextInputType.number)),
        const SizedBox(width: 5),
        Expanded(child: TextField(controller: nDR, decoration: const InputDecoration(labelText: "Disc ₹"), keyboardType: TextInputType.number)),
      ]),
      const SizedBox(height: 10),
