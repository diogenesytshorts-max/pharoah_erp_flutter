  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        ...
        if (search.isNotEmpty && selectedMed == null) Positioned(top: 70, left: 10, right: 10, child: Material(elevation: 5, child: Container(constraints: const BoxConstraints(maxHeight: 250), child: ListView(shrinkWrap: true, children: ph.medicines.where((m) => m.name.toLowerCase().contains(search.toLowerCase())).map((m) => ListTile(title: Text(m.name), subtitle: Text("Stock: ${m.stock}"), onTap: () => setState(() { selectedMed = m; search = ""; }))).toList()))))
      ]),
    );
  }
...
class _ItemEntryFormState extends State<ItemEntryForm> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Container(padding: const EdgeInsets.all(15), color: Colors.white, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(children: [
        if (rateType == 'C') Expanded(child: TextField(controller: rateCDiscC, decoration: const InputDecoration(labelText: "Rate C Disc %"), keyboardType: TextInputType.number, onChanged: (v)=>_updateRate())),
      ...
      ElevatedButton(style: ElevatedButton.styleFrom(minimumSize: const Size(double.infinity, 45), backgroundColor: Colors.green), onPressed: () {
        double r = double.tryParse(rateC.text) ?? 0; double q = double.tryParse(qtyC.text) ?? 0; double d = double.tryParse(normalDiscC.text) ?? 0; double g = double.tryParse(gstC.text) ?? 0;
