...
class _BillingViewState extends State<BillingView> {
          ...
          Column(
            children: [
              if (selectedMed == null && !widget.isReadOnly)
                Padding(padding: const EdgeInsets.all(12), child: TextField(autofocus: true, decoration: const InputDecoration(hintText: "Search Medicine...", prefixIcon: Icon(Icons.search), border: OutlineInputBorder()), onChanged: (v) => setState(() => search = v))),
              if (selectedMed != null)
                ItemEntryForm(
                  med: selectedMed!, partyState: widget.party.state, shopState: ph.companyState,
                  srNo: editingIndex ?? items.length + 1, existingItem: editingIndex != null ? items[editingIndex!] : null,
              ...
              Expanded(
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) => ListTile(
                    ...
                    subtitle: Text("Qty: ${items[index].qty.toInt()} | Batch: ${items[index].batch} | Exp: ${items[index].exp}"),
                    trailing: Text("₹${items[index].total.toStringAsFixed(2)}", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)),
  ...
  void _saveAndClose(PharoahManager ph) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("✅ Sale Invoice Saved Successfully!"), backgroundColor: Colors.green));
  ...
  void _saveAndPrint(PharoahManager ph) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("✅ Invoice Saved & Printed!"), backgroundColor: Colors.blue));
    Navigator.of(context).popUntil((route) => route.isFirst);
  }
}
...
class _ItemEntryFormState extends State<ItemEntryForm> {
  ...
  @override Widget build(BuildContext context) {
    return Container(padding: const EdgeInsets.all(15), color: Colors.blue.shade50, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (widget.batchHistory.isNotEmpty) ...[
        const Text("Select Batch:", style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
      ...
      ElevatedButton(style: ElevatedButton.styleFrom(minimumSize: const Size(double.infinity, 50), backgroundColor: Colors.blue.shade700), onPressed: () async {
        if (originalExp != null && originalExp != eC.text) {
          bool c = await showDialog(context: context, builder: (c) => AlertDialog(title: const Text("Update Expiry?"), content: const Text("Expiry change ho rahi hai, continue?"), actions: [TextButton(onPressed: () => Navigator.pop(c, false), child: const Text("NO")), TextButton(onPressed: () => Navigator.pop(c, true), child: const Text("YES"))])) ?? false;
          if (!c) return;
        }
  ...
  Widget _f(TextEditingController c, String l, {List<TextInputFormatter>? fmt}) => TextField(controller: c, inputFormatters: fmt, decoration: InputDecoration(labelText: l, border: const OutlineInputBorder(), contentPadding: const EdgeInsets.all(8)));
}
