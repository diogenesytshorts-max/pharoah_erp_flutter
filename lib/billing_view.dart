    ...
    return Scaffold(
      ...
      body: Stack(children: [
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (c, i) => ListTile(
                ...
                subtitle: Text("Qty: ${items[i].qty.toInt()} | B: ${items[i].batch} | R: ${items[i].rate}"),
                trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                  Text("₹${items[i].total.toStringAsFixed(2)}", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)),
                  if (!widget.isReadOnly) ...[
                    IconButton(icon: const Icon(Icons.copy, size: 18, color: Colors.orange), onPressed: () {
                      setState(() {
        ...
        if (search.isNotEmpty && selectedMed == null)
          Positioned(top: 70, left: 10, right: 10, child: Material(elevation: 5, child: Container(
            constraints: const BoxConstraints(maxHeight: 250),
            child: ListView(
              shrinkWrap: true,
              children: ph.medicines.where((m) => m.name.toLowerCase().contains(search.toLowerCase())).map((m) => ListTile(
                title: Text(m.name),
...
// =====================================================================
// --- ITEM ENTRY FORM (CALCULATION & BATCH HISTORY) ---
// =====================================================================
...
class _ItemEntryFormState extends State<ItemEntryForm> {
  ...
  final nDP = TextEditingController(text: "0.0"); // Normal Disc %
  final nDR = TextEditingController(text: "0.0"); // Normal Disc ₹
  String rT = "A";
  ...
  void initState() {
    ...
    // Auto-Slash for MM/YY and month validation
    eC.addListener(() {
  ...
  void _calculateRateC() {
    ...
    // Formula: Rate C = (MRP / (1 + GST/100)) - (Rate C Disc %)
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (history.isNotEmpty)
          SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: history.map((b) => ActionChip(
            label: Text(b.batch),
            onPressed: () {
              setState(() {
        ...
        SegmentedButton<String>(
          ...
          selected: {rT},
          onSelectionChanged: (val) { setState(() => rT = val.first); _updateRateType(); },
        ),
        const SizedBox(height: 5),
        Row(children: [
          if (rT == 'C')
            Expanded(child: TextField(controller: rCD, decoration: const InputDecoration(labelText: "RC Disc%", labelStyle: TextStyle(color: Colors.purple, fontSize: 12)), keyboardType: TextInputType.number, onChanged: (v) => _calculateRateC())),
