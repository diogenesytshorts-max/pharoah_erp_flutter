...
class _ChallanStitcherWizardState extends State<ChallanStitcherWizard> with SingleTickerProviderStateMixin {
  ...
  void _resetWizard() {
    setState(() { funnelStep = "MODE"; selectedRoute = null; selectedPartyNames.clear(); draftBills.clear(); });
  }
  ...
  Future<void> _handleBatchSave(PharoahManager ph) async {
    ...
    setState(() { isProcessing = true; progressText = "Finalizing Bills..."; });
  ...
  Future<void> _handleZipExport(PharoahManager ph) async {
    ...
    for (var draft in selected) {
      try {
        final saleObj = ph.sales.firstWhere((s) => s.billNo == draft['billNo']);
        finalPayload.add({'saleObj': saleObj, 'party': draft['party']});
      } catch (e) { continue; }
    }

    setState(() { isProcessing = true; progressValue = 0.0; });
    try {
      String path = await BulkPdfService.createBillsZip(selectedDrafts: finalPayload, shop: ph.activeCompany!, onProgress: (v, n) => setState(() { progressValue = v; progressText = "Packing: $n"; }));
      setState(() => isProcessing = false);
      await Share.shareXFiles([XFile(path)], subject: 'Batch_Bills');
    } catch (e) { setState(() => isProcessing = false); }
  }

  // ===========================================================================
  // 🖥️ UI: THE FRESH MODERN LOOK
  // ===========================================================================
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        _buildStepRouter(ph, primaryColor),
  ...
  Widget _stepMode(PharoahManager ph, Color color) {
    return Center(child: Padding(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        _modeCard("MONTHLY BATCH", "Select any month from Apr to Mar", Icons.calendar_month, color, () => _showMonthPicker(ph.currentFY)),
        const SizedBox(height: 20),
        _modeCard("RANDOM RANGE", "Select custom dates in $selectionMode", Icons.date_range, Colors.blueGrey, () => _pickRandom(ph.currentFY)),
      ]),
    ));
  }
  ...
  Widget _stepParty(PharoahManager ph, Color color) {
    ...
    return Column(children: [
      _header("STEP 3: SELECT PARTIES TO STITCH"),
      Container(padding: const EdgeInsets.all(12), color: Colors.white, child: Row(children: [
        ...
        TextButton(onPressed: () => setState(() => selectedPartyNames = list.map((e)=>e.name).toList()), child: const Text("ALL")),
      ])),
  ...
  Widget _iconAct(IconData i, String l, Color c, VoidCallback onTap) => InkWell(onTap: onTap, child: Column(children: [Icon(i, color: c, size: 20), Text(l, style: TextStyle(fontSize: 8, color: c, fontWeight: FontWeight.bold))]));
  ...
  void _generateDrafts(PharoahManager ph) {
    ...
    for (var pName in selectedPartyNames) {
      ...
      temp.add({'party': pObj, 'billNo': 'DRAFT', 'date': toDate, 'items': items, 'total': items.fold(0.0, (s, i)=>s+i.total), 'status': 'DRAFT', 'isSelected': true, 'challanIds': chs.map((c)=>c.id).toList()});
    }
    setState(() { draftBills = temp; funnelStep = "REVIEW"; isProcessing = false; });
  }

  void _saveSingle(PharoahManager ph, int i) async {
    setState(() => isProcessing = true);
    var b = draftBills[i];
