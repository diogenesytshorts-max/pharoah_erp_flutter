...
class _WebGatewayMasterState extends State<WebGatewayMaster> {
  ...
  Future<void> _checkGoogleSession() async {
    ...
    if (mounted) setState(() => connectedEmail = email);
  }
  ...
  void _showNewSale(BuildContext context, WebManager web) {
    ...
    showDialog(context: context, builder: (c) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 10),
        DropdownButtonFormField<Medicine>(initialValue: m, decoration: const InputDecoration(labelText: "Item"), items: web.medicines.map((e) => DropdownMenuItem(value: e, child: Text(m.name + " (Stock: " + m.stock.toInt().toString() + ")"))).toList(), onChanged: (v) => m = v!),
        const SizedBox(height: 10),
